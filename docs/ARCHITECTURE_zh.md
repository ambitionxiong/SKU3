...
- 键位处理在 `simulator/main.c`（SDL 键盘轮询循环），分两类：

### 4.1 导航/功能键（走业务按键分发 nav_handle_key → process_key）

| 键盘键 | 内部键值 | 作用 |
|--------|---------|------|
| `Tab` | KEY_MENU | 菜单键（进入/确认） |
| `Esc` | KEY_BACK | 返回（页面栈 pop） |
| `→` / `↓` | KEY_ENCODER_CW | 编码器正转（焦点下一项） |
| `←` / `↑` | KEY_ENCODER_CCW | 编码器反转（焦点上一项） |
| `空格` | KEY_ENCODER_PRESS | 编码器按下（确认/选中） |
| `1` | KEY1 | 功能键 1 |
| `2` | KEY_SIXMENU | 第六感菜单 |
| `4` | KEY_PREHEAT | 预热入口 |
| `5` | KEY_EXTRA_COLOR | 额外上色 |
| `7` | KEY_CLEAN | 清洁入口 |
| `F9` | KEY_SET | 设置页 |

### 4.2 协议模拟键（直接改 uart_data_receive 模拟串口数据，**不进业务按键分发**）

| 键盘键 | 作用 |
|--------|------|
| `0` | 门开/关（Power_ALL_State bit1 翻转） |
| `8` | 探针温度 +5℃ |
| `-`（减号）| 探针温度 -5℃ |
| `9` | 探针插拔切换（bit2 翻转） |
| `6` | **中英切换**（g_lang_en 取反 + 立即刷新当前屏，不进入任何业务键处理） |

> 说明：`0/8/9/-` 直接改写串口协议数组 `uart_data_receive`，模拟硬件上报；
> `6` 是语言标志位切换，只触发显示层刷新——两者都与业务按键逻辑无关（这也保证
> 语言切换不影响事件逻辑，见 I18N_GUIDE_zh.md §7）。
> 键位备注：`3` 未使用；`6` 为语言专用，不映射业务键。
