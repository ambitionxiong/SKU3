...
int chip_setting_saved_hour, chip_setting_saved_min;

static void on_chip_menu_next_click(lv_event_t *e);
static void on_chip_set_sure_click(lv_event_t *e);
static void on_chip_cooking_stop_click(lv_event_t *e);
...
static void chip_delay_toggle(lv_event_t *e);
static void chip_contain_toggle(lv_event_t *e);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 薯条 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 薯条 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
void jump_to_chip_menu(void)
{
    ...
    page_push(PAGE_CHIP_MENU);
    lv_obj_clean(lv_scr_act());
    chip_menu_create(&ui_manager);
    ...
    if (menu) {
        ...
        if (g_chip_menu) lv_group_del(g_chip_menu);
        g_chip_menu = group_create_for_page(btns, 3);
        ...
        nav_blink_extra(menu->hour, menu->label_807);
        nav_blink_extra(menu->min, menu->label_808);

        lv_obj_add_event_cb(menu->hour, on_chip_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min, on_chip_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_chip_edit_focus,
                            LV_EVENT_FOCUSED, NULL);

        if (menu->next)
            lv_obj_add_event_cb(menu->next, on_chip_menu_next_click,
...
{
    g_delay_source_page = PAGE_CHIP_SET;   /* 记录来源子类：收藏卡片名/启动分发依赖 */
    page_push(PAGE_CHIP_SET);
    ...
    if (set) {
        ...
        if (g_chip_set) lv_group_del(g_chip_set);
        g_chip_set = group_create_for_page(btns, 5);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offdelay, chip_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, chip_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offcontain, chip_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->oncontain, chip_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->sure, on_chip_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
    }
...
void jump_to_chip_cooking(void)
{
    ...
    if (cook) {
        ...
        if (g_chip_cooking) lv_group_del(g_chip_cooking);
        g_chip_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_chip_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->little, on_chip_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        chip_set_status(cook->status, set_temp, set_hour, set_min);
        lv_label_set_text_fmt(cook->timelabel, "%02d:%02d:%02d", set_hour, set_min, 0);
    }

    cook_total_ms = (set_hour * 3600 + set_min * 60) * 1000;
    if (cook) {
        lv_bar_set_range(cook->bar_64, 0, 100);
        lv_bar_set_value(cook->bar_64, 3, LV_ANIM_OFF);

        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, cook->bar_64);
...
void jump_to_chip_setting(void)
{
    ...
    chip_setting_saved_hour = set_hour;
    chip_setting_saved_min = set_min;

    page_push(PAGE_CHIP_SETTING);
    lv_obj_clean(lv_scr_act());
    chip_setting_create(&ui_manager);

    chip_setting_t *set = chip_setting_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->hour, set->min, set->sure };
        if (g_chip_setting) lv_group_del(g_chip_setting);
        g_chip_setting = group_create_for_page(btns, 3);
        ...
        edit_register(set->min, set->minline, NULL,
                      &set_min, 0, 59, 1, "%02d");
        /* blink extras: 单位/方向箭头(离线推导,最近值归属) */
        nav_blink_extra(set->hour, set->label_834);
        nav_blink_extra(set->min, set->label_836);

        lv_obj_add_event_cb(set->hour, on_chip_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_chip_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_chip_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_chip_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
...
void jump_to_chip_stop(void)
{
    ...
    if (stop) {
        ...
        if (g_chip_stop) lv_group_del(g_chip_stop);
        g_chip_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_chip_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_chip_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        chip_set_status(stop->status, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_65, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
...
void jump_to_chip_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_CHIP_COOKING) {
        chip_cooking_t *cook = chip_cooking_get(&ui_manager);
        if (cook && cook->bar_64) cooking_bar_val = lv_bar_get_value(cook->bar_64);
    }
    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_chip_complete;
    page_push(PAGE_CHIP_STOP_BACK);
    lv_obj_clean(lv_scr_act());
    ...
    if (back) {
        ...
        if (p > 100) p = 100;
        lv_bar_set_range(back->bar_66, 0, 100);
        lv_bar_set_value(back->bar_66, p, LV_ANIM_OFF);
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_841, tr("烹饪中..."));

        if (g_complete_to_stop_back) {
            g_complete_to_stop_back = 0;
            if (g_keepwarm_active)
                lv_label_set_text(back->label_841, tr("保温中..."));
            else
                lv_label_set_text(back->label_841, tr("已完成"));
            lv_bar_set_value(back->bar_66, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_841, tr("预约中..."));
            lv_label_set_text(back->label_843, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_66, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_384, LV_OBJ_FLAG_HIDDEN);
...
void chip_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_64, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_64, cook_bar_saved, LV_ANIM_OFF);
...
void jump_to_chip_complete(void)
{
    ...
    if (depth > 0 && page_stack[depth - 1] == PAGE_CHIP_STOP)
        depth--;
    page_push(PAGE_CHIP_COMPLETE);
    lv_obj_clean(lv_scr_act());
    chip_complete_create(&ui_manager);

    {
        chip_complete_t *cook = chip_complete_get(&ui_manager);
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_chip_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            chip_set_status(cook->status, set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_67, 100, LV_ANIM_OFF);
        }
    }
...
void chip_rebuild_set(page_id_t child)
{
    ...
    if (set_hour == 0) {
        ...
        if (child == PAGE_CHIP_COOKING && set->sure)
            lv_group_focus_obj(set->sure);
    }
...
void chip_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (child == PAGE_CHIP_SETTING) {
            ...
            lv_bar_set_value(cook->bar_64, progress, LV_ANIM_OFF);
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, cook->bar_64);
            lv_anim_set_exec_cb(&a, anim_bar_set_value);
...
void chip_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->hour, on_chip_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_chip_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_chip_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_chip_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
...
{
    lv_obj_add_event_cb(btn, on_chip_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
