...
static void on_hot_bbq_probe_delay_toggle(lv_event_t *e);
...
void jump_to_hot_bbq_menu_probe(void)
{
    ...
    lang_scr_load_anim(hot_bbq_menu_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_set_probe(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->sure, on_hot_bbq_probe_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offdelay, on_hot_bbq_probe_delay_toggle,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, on_hot_bbq_probe_delay_toggle,
                            LV_EVENT_CLICKED, NULL);
    }
    current_group = g_hot_bbq_set_probe;

    lang_scr_load_anim(hot_bbq_set_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_cooking_probe(void)
{
    ...
    if (cook) {
        ...
        lv_label_set_text_fmt(cook->status, tr("| 热风烧烤 | %d℃ | %d℃"), set_temp, probe_target_temp);
    ...
    lang_scr_load_anim(hot_bbq_cooking_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_stop_probe(void)
{
    ...
    if (stop) {
        ...
        lv_label_set_text_fmt(stop->status, tr("| 热风烧烤 | %d℃ | %d℃"), set_temp, probe_target_temp);
    ...
    lang_scr_load_anim(hot_bbq_stop_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_stop_back_probe(void)
{
    ...
    if (back) {
        ...
        lv_label_set_text_fmt(back->status, tr("| 热风烧烤 | %d℃ | %d℃"), set_temp, probe_target_temp);

        lv_bar_set_range(back->bar_3, 0, 100);
        lv_bar_set_value(back->bar_3, bar_val, LV_ANIM_OFF);
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_39, tr("烹饪中..."));

        if (g_complete_to_stop_back) {
            g_complete_to_stop_back = 0;
            lv_label_set_text(back->label_39, tr("已完成"));
            lv_bar_set_value(back->bar_3, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_39, tr("预约中..."));
            lv_label_set_text(back->label_41, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
    ...
    lang_scr_load_anim(hot_bbq_stop_back_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...

    {
        ...
        if (complete) {
            ...
            lv_obj_add_event_cb(complete->image_15, on_hot_bbq_probe_complete_click,
                                LV_EVENT_CLICKED, NULL);
            lv_label_set_text_fmt(complete->status, tr("| 热风烧烤 | %d℃ | %d℃"), set_temp, probe_target_temp);
