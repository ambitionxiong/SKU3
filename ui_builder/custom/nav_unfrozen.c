...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, "| 解冻 | %d℃ | %02d分钟", temp, min);
    else
        lv_label_set_text_fmt(label, "| 解冻 | %d℃ | %d小时%02d分钟", temp, hour, min);
}
...
void jump_to_unfrozen_menu(void)
{
    ...
    printf("[pizza_2] jump: special_menu -> unfrozen_menu\n");
}
...
void jump_to_unfrozen_set(void)
{
    ...
    lv_scr_load_anim(unfrozen_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] jump: menu -> unfrozen_set\n");
}
...
void jump_to_unfrozen_cooking(void)
{
    ...
    printf("[pizza_2] jump: set -> unfrozen_cooking\n");
}
...
void jump_to_unfrozen_setting(void)
{
    ...
    printf("[pizza_2] jump: cooking -> unfrozen_setting\n");
}
    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_480, "保温中...");
            else
                lv_label_set_text(back->label_480, "已完成");
            lv_bar_set_value(back->bar_34, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_480, "预约中...");
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_480, "烹饪中...");
    }
...

    {
        ...
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_unfrozen_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, "| 解冻 | %d℃ | %02d分钟", set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, "| 解冻 | %d℃ | %d小时%02d分钟", set_temp, set_hour, set_min);
...
void unfrozen_rebuild_menu(page_id_t child)
{
    ...
    lv_scr_load_anim(unfrozen_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to unfrozen_menu\n");
}
...
void unfrozen_rebuild_set(page_id_t child)
{
    ...
    lv_scr_load_anim(unfrozen_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to unfrozen_set\n");
}
...
void unfrozen_rebuild_cooking(page_id_t child)
{
    ...
    lv_scr_load_anim(unfrozen_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to unfrozen_cooking\n");
}
...
void unfrozen_rebuild_setting(void)
{
    ...
    lv_scr_load_anim(unfrozen_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to unfrozen_setting\n");
}
...
void unfrozen_rebuild_stop(void)
{
    ...
    lv_scr_load_anim(unfrozen_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to unfrozen_stop\n");
    g_send.iface_status = IFACE_PAUSE;
}
...
void unfrozen_rebuild_stop_back(void)
{
    ...
    lv_scr_load_anim(unfrozen_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to unfrozen_stop_back\n");
}
...
void unfrozen_rebuild_complete(void)
{
    ...
    lv_scr_load_anim(unfrozen_complete_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to unfrozen_complete\n");
}
