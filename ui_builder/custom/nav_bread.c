...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, "| 面包 | %d℃ | %02d分钟", temp, min);
    else
        lv_label_set_text_fmt(label, "| 面包 | %d℃ | %d小时%02d分钟", temp, hour, min);
}
