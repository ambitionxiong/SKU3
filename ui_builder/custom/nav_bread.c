...
static void bread_set_status(lv_obj_t *label, int temp, int hour, int min);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 面包 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 面包 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
void jump_to_bread_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->hour, menu->label_725);
        nav_blink_extra(menu->min, menu->label_726);
    ...
    lang_scr_load_anim(bread_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
{
    g_delay_source_page = PAGE_BREAD_SET;   /* 记录来源子类：收藏卡片名/启动分发依赖 */
    ...
    lang_scr_load_anim(bread_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bread_cooking(void)
{
    ...
    if (cook) {
        ...
        bread_set_status(cook->status, set_temp, set_hour, set_min);
    ...
    lang_scr_load_anim(bread_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bread_setting(void)
{
    ...
    if (set) {
        ...
        edit_register(set->min, set->minline, NULL,
                      &set_min, 0, 59, 1, "%02d");
        /* blink extras: 单位/方向箭头(离线推导,最近值归属) */
        nav_blink_extra(set->hour, set->label_752);
        nav_blink_extra(set->min, set->label_754);
    ...
    lang_scr_load_anim(bread_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bread_stop(void)
{
    ...
    if (stop) {
        ...
        bread_set_status(stop->status, set_temp, set_hour, set_min);
    ...
    lang_scr_load_anim(bread_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bread_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_BREAD_COOKING) {
        bread_cooking_t *cook = bread_cooking_get(&ui_manager);
        if (cook && cook->bar_56) cooking_bar_val = lv_bar_get_value(cook->bar_56);
    }
    g_on_stop_back = 1;
    ...
    if (back) {
        ...
        g_bread_stop_back = group_create_for_page(btns, 1);
        lv_obj_add_event_cb(back->sure, on_bread_stop_back_sure_click,
                            LV_EVENT_CLICKED, NULL);

        bread_set_status(back->status, set_temp, set_hour, set_min);
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_759, tr("烹饪中..."));

        if (g_complete_to_stop_back) {
            g_complete_to_stop_back = 0;
            if (g_keepwarm_active)
                lv_label_set_text(back->label_759, tr("保温中..."));
            else
                lv_label_set_text(back->label_759, tr("已完成"));
            lv_bar_set_value(back->bar_58, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_759, tr("预约中..."));
            lv_label_set_text(back->label_761, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_58, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_352, LV_OBJ_FLAG_HIDDEN);
    ...
    lang_scr_load_anim(bread_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...

    {
        ...
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_bread_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            bread_set_status(cook->status, set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_59, 100, LV_ANIM_OFF);
        }
    }
...
{
    lv_obj_add_event_cb(btn, on_bread_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
