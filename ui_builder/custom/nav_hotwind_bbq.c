...
int hotwind_bbq_setting_saved_temp, hotwind_bbq_setting_saved_hour, hotwind_bbq_setting_saved_min;


static void on_hotwind_bbq_menu_next_click(lv_event_t *e);
static void on_hotwind_bbq_set_sure_click(lv_event_t *e);
static void on_hotwind_bbq_cooking_stop_click(lv_event_t *e);
static void on_hotwind_bbq_cooking_setting_click(lv_event_t *e);
static void on_hotwind_bbq_setting_sure_click(lv_event_t *e);
static void on_hotwind_bbq_stop_start_click(lv_event_t *e);
void hotwind_bbq_resume_cooking(void);
static void on_hotwind_bbq_stop_back_sure_click(lv_event_t *e);
static void on_hotwind_bbq_edit_focus(lv_event_t *e);
...
static void hotwind_bbq_delay_toggle(lv_event_t *e);
static void hotwind_bbq_contain_toggle(lv_event_t *e);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 热风 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 热风 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
static void on_hotwind_bbq_stop_back_sure_click(lv_event_t *e)
{
    ...
    printf("[hotwind_bbq] stop_back sure -> major_menu\n");
}
...
void jump_to_hotwind_bbq_menu(void)
{
    ...
    if (menu) {
        ...
        if (g_hotwind_bbq_menu) lv_group_del(g_hotwind_bbq_menu);
        g_hotwind_bbq_menu = group_create_for_page(btns, 4);
        ...
        nav_blink_extra(menu->temp, menu->label_207);
        nav_blink_extra(menu->hour, menu->label_210);
        nav_blink_extra(menu->min, menu->label_211);

        lv_obj_add_event_cb(menu->temp, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->hour, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);

        if (menu->next)
            lv_obj_add_event_cb(menu->next, on_hotwind_bbq_menu_next_click,
    ...
    current_group = g_hotwind_bbq_menu;

    lang_scr_load_anim(hotwind_bbq_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        g_send.cook_mode = MODE_HOTWIND_BBQ;
    printf("[hotwind_bbq] jump: cookmenu -> hotwind_bbq_menu\n");
}

// menu → set
void jump_to_hotwind_bbq_set(void)
{
    page_push(PAGE_HOTWIND_BBQ_SET);
    lv_obj_clean(lv_scr_act());
    ...
    if (set) {
        ...
        if (g_hotwind_bbq_set) lv_group_del(g_hotwind_bbq_set);
        g_hotwind_bbq_set = group_create_for_page(btns, 7);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_flag(set->label_221, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 312, 254);
        lv_obj_set_pos(set->label_222, 365, 269);
    } else {
        lv_obj_clear_flag(set->hour, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(set->label_221, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 395, 254);
        lv_obj_set_pos(set->label_222, 448, 269);
    }
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offpreheat, hotwind_bbq_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->onpreheat, hotwind_bbq_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offdelay, hotwind_bbq_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, hotwind_bbq_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offcontain, hotwind_bbq_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->oncontain, hotwind_bbq_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->sure, on_hotwind_bbq_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
    }
    current_group = g_hotwind_bbq_set;

    lang_scr_load_anim(hotwind_bbq_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] jump: menu -> hotwind_bbq_set\n");
}
...
void jump_to_hotwind_bbq_cooking(void)
{
    ...
    page_push(PAGE_HOTWIND_BBQ_COOKING);
    lv_obj_clean(lv_scr_act());
    hotwind_bbq_cooking_create(&ui_manager);

    hotwind_bbq_cooking_t *cook = hotwind_bbq_cooking_get(&ui_manager);
    if (cook) {
        lv_obj_t *btns[] = { cook->stop, cook->little };
        if (g_hotwind_bbq_cooking) lv_group_del(g_hotwind_bbq_cooking);
        g_hotwind_bbq_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_hotwind_bbq_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->little, on_hotwind_bbq_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        hotwind_bbq_set_status(cook->status, set_temp, set_hour, set_min);
        lv_label_set_text_fmt(cook->timelabel, "%02d:%02d:%02d", set_hour, set_min, 0);
    }

    cook_total_ms = (set_hour * 3600 + set_min * 60) * 1000;
    if (cook) {
        lv_bar_set_range(cook->bar_18, 0, 100);
        lv_bar_set_value(cook->bar_18, 3, LV_ANIM_OFF);

        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, cook->bar_18);
    ...
    current_group = g_hotwind_bbq_cooking;

    lang_scr_load_anim(hotwind_bbq_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[hotwind_bbq] jump: set -> hotwind_bbq_cooking\n");
}

// cooking → setting（不暂停 timer）
void jump_to_hotwind_bbq_setting(void)
{
    hotwind_bbq_setting_saved_temp = set_temp; hotwind_bbq_setting_saved_hour = set_hour; hotwind_bbq_setting_saved_min = set_min;
    page_push(PAGE_HOTWIND_BBQ_SETTING);
    lv_obj_clean(lv_scr_act());
    hotwind_bbq_setting_create(&ui_manager);

    hotwind_bbq_setting_t *set = hotwind_bbq_setting_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->temp, set->hour, set->min, set->sure };
        if (g_hotwind_bbq_setting) lv_group_del(g_hotwind_bbq_setting);
        g_hotwind_bbq_setting = group_create_for_page(btns, 4);
        ...
        nav_blink_extra(set->hour, set->label_8);
        nav_blink_extra(set->min, set->label_9);

        lv_obj_add_event_cb(set->temp, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_hotwind_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_hotwind_bbq_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    current_group = g_hotwind_bbq_setting;

    lang_scr_load_anim(hotwind_bbq_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        if (g_send.iface_status != IFACE_COMPLETE)
        g_send.iface_status = (cook_timer != NULL) ? IFACE_COOKING : IFACE_SETTING;
    printf("[hotwind_bbq] jump: cooking -> hotwind_bbq_setting\n");
}
...
void jump_to_hotwind_bbq_stop(void)
{
    ...
    page_push(PAGE_HOTWIND_BBQ_STOP);
    lv_obj_clean(lv_scr_act());
    hotwind_bbq_stop_create(&ui_manager);

    hotwind_bbq_stop_t *stop = hotwind_bbq_stop_get(&ui_manager);
    if (stop) {
        lv_obj_t *btns[] = { stop->start, stop->little };
        if (g_hotwind_bbq_stop) lv_group_del(g_hotwind_bbq_stop);
        g_hotwind_bbq_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_hotwind_bbq_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_hotwind_bbq_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        hotwind_bbq_set_status(stop->status, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_19, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(stop->bar_19, cook_bar_saved, LV_ANIM_OFF);
    }
    current_group = g_hotwind_bbq_stop;

    lang_scr_load_anim(hotwind_bbq_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[hotwind_bbq] jump: cooking -> stop (pause)\n");
}
...
void jump_to_hotwind_bbq_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_HOTWIND_BBQ_COOKING) {
        hotwind_bbq_cooking_t *cook = hotwind_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_18) cooking_bar_val = lv_bar_get_value(cook->bar_18);
    }

    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_hotwind_bbq_complete;
    page_push(PAGE_HOTWIND_BBQ_STOP_BACK);
    lv_obj_clean(lv_scr_act());
    hotwind_bbq_stop_back_create(&ui_manager);

    hotwind_bbq_stop_back_t *back = hotwind_bbq_stop_back_get(&ui_manager);
    if (back) {
        lv_obj_t *btns[] = { back->sure };
        if (g_hotwind_bbq_stop_back) lv_group_del(g_hotwind_bbq_stop_back);
        g_hotwind_bbq_stop_back = group_create_for_page(btns, 1);
        lv_obj_add_event_cb(back->sure, on_hotwind_bbq_stop_back_sure_click,
                            LV_EVENT_CLICKED, NULL);

        hotwind_bbq_set_status(back->status, set_temp, set_hour, set_min);
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_238, tr("保温中..."));
            else
                lv_label_set_text(back->label_238, tr("已完成"));
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_238, tr("烹饪中..."));
    }
    current_group = g_hotwind_bbq_stop_back;

    lang_scr_load_anim(hotwind_bbq_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] jump: stop/cooking -> stop_back\n");
}
...
void hotwind_bbq_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_18, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_18, cook_bar_saved, LV_ANIM_OFF);
    ...
    printf("[hotwind_bbq] resume: stop -> cooking\n");
}
...
static void on_hotwind_bbq_setting_sure_click(lv_event_t *e)
{
    ...
    if (depth > 1) depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTWIND_BBQ_STOP)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTWIND_BBQ_COMPLETE)
        depth--;
    lv_obj_clean(lv_scr_act());
    hotwind_bbq_cooking_create(&ui_manager);
    ...
    g_send.set_temp = set_temp;
    g_send.remaining_ms = cook_total_ms;
    printf("[hotwind_bbq] setting sure -> cooking\n");
}
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTWIND_BBQ_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTWIND_BBQ_STOP)
        depth--;
    page_push(PAGE_HOTWIND_BBQ_COMPLETE);
    lv_obj_clean(lv_scr_act());
    hotwind_bbq_complete_create(&ui_manager);

    {
        hotwind_bbq_complete_t *cook = hotwind_bbq_complete_get(&ui_manager);
        if (cook) {
            lv_obj_t *btns[] = { cook->button_74 };
            if (g_hotwind_bbq_complete) lv_group_del(g_hotwind_bbq_complete);
            g_hotwind_bbq_complete = group_create_for_page(btns, 1);
            lv_obj_add_event_cb(cook->button_74, on_hotwind_bbq_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            hotwind_bbq_set_status(cook->status, set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_21, 100, LV_ANIM_OFF);
        }
    }
    ...
    g_send.remaining_ms = 0;
        g_send.iface_status = IFACE_COMPLETE;
    g_send.remaining_ms = 0;
    printf("[hotwind_bbq] jump: cooking -> complete\n");
...
void hotwind_bbq_rebuild_menu(page_id_t child)
{
    ...
    lang_scr_load_anim(hotwind_bbq_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] back to hotwind_bbq_menu\n");
}
...
void hotwind_bbq_rebuild_set(page_id_t child)
{
    ...
    if (set_hour == 0) {
        ...
        if (child == PAGE_HOTWIND_BBQ_COOKING && set->sure)
            lv_group_focus_obj(set->sure);
    }
    ...
    lang_scr_load_anim(hotwind_bbq_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] back to hotwind_bbq_set\n");
}
...
void hotwind_bbq_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (child == PAGE_HOTWIND_BBQ_SETTING) {
            ...
            lv_bar_set_value(cook->bar_18, progress, LV_ANIM_OFF);
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, cook->bar_18);
            lv_anim_set_exec_cb(&a, anim_bar_set_value);
    ...
    lang_scr_load_anim(hotwind_bbq_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] back to hotwind_bbq_cooking\n");
}
...
void hotwind_bbq_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->temp, on_hotwind_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_hotwind_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_hotwind_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_hotwind_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_hotwind_bbq_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    lang_scr_load_anim(hotwind_bbq_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] back to hotwind_bbq_setting\n");
}
...
void hotwind_bbq_rebuild_stop(void)
{
    ...
    lang_scr_load_anim(hotwind_bbq_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] back to hotwind_bbq_stop\n");
    g_send.iface_status = IFACE_PAUSE;
}
...
void hotwind_bbq_rebuild_stop_back(void)
{
    ...
    if (back) {
        ...
        if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_HOTWIND_BBQ_COOKING) {
        hotwind_bbq_cooking_t *cook = hotwind_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_18) p = lv_bar_get_value(cook->bar_18);
    }
        if (p <= 0) {
            uint32_t elapsed = cook_timer ? (lv_tick_get() - cook_start_time) : cook_elapsed_saved;
            p = stop_back_progress(elapsed, cook_total_ms);
    ...
    lang_scr_load_anim(hotwind_bbq_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hotwind_bbq] back to hotwind_bbq_stop_back\n");
}
