...
int cookie_setting_saved_temp, cookie_setting_saved_hour, cookie_setting_saved_min;
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 饼干 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 饼干 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
void jump_to_cookie_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->temp, menu->label_90);
        nav_blink_extra(menu->hour, menu->label_93);
        nav_blink_extra(menu->min, menu->label_94);
...
void jump_to_cookie_set(void)
{
    ...
    if (set) {
        ...
        g_cookie_set = group_create_for_page(btns, 5);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_flag(set->label_104, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 312, 254);
        lv_obj_set_pos(set->label_105, 365, 269);
    } else {
        lv_obj_clear_flag(set->hour, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(set->label_104, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 395, 254);
        lv_obj_set_pos(set->label_105, 448, 269);
    }
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offdelay, cookie_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, cookie_delay_toggle, LV_EVENT_CLICKED, NULL);
...
{
    cookie_setting_saved_temp = set_temp; cookie_setting_saved_hour = set_hour; cookie_setting_saved_min = set_min;
    ...
    if (set) {
        ...
        nav_blink_extra(set->hour, set->label_132);
        nav_blink_extra(set->min, set->label_133);
...
void jump_to_cookie_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_COOKIE_COOKING) {
        cookie_cooking_t *cook = cookie_cooking_get(&ui_manager);
        if (cook && cook->bar_4) cooking_bar_val = lv_bar_get_value(cook->bar_4);
    }
    ...
    if (back) {
        ...
        if (g_delay_cancel_to_stop_back) {
            ...
            lv_label_set_text(back->label_141, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_6, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_68, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->button_29, LV_OBJ_FLAG_HIDDEN);
        }
...

    {
        ...
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_cookie_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, tr("| 饼干 | %d℃ | %02d分钟"), set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, tr("| 饼干 | %d℃ | %d小时%02d分钟"), set_temp, set_hour, set_min);
...
{
    lv_obj_add_event_cb(btn, on_cookie_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
