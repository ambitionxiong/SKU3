...
int windchange_bbq_setting_saved_temp, windchange_bbq_setting_saved_hour, windchange_bbq_setting_saved_min;
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 热风对流 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 热风对流 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
void jump_to_windchange_bbq_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->temp, menu->label_321);
        nav_blink_extra(menu->hour, menu->label_324);
        nav_blink_extra(menu->min, menu->label_325);
    ...
    lang_scr_load_anim(windchange_bbq_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_windchange_bbq_set(void)
{
    ...
    lang_scr_load_anim(windchange_bbq_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_windchange_bbq_cooking(void)
{
    ...
    lang_scr_load_anim(windchange_bbq_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
{
    windchange_bbq_setting_saved_temp = set_temp; windchange_bbq_setting_saved_hour = set_hour; windchange_bbq_setting_saved_min = set_min;
    ...
    if (set) {
        ...
        nav_blink_extra(set->hour, set->label_38);
        nav_blink_extra(set->min, set->label_39);
    ...
    lang_scr_load_anim(windchange_bbq_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_windchange_bbq_stop(void)
{
    ...
    lang_scr_load_anim(windchange_bbq_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_windchange_bbq_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_WINDCHANGE_BBQ_COOKING) {
        windchange_bbq_cooking_t *cook = windchange_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_30) cooking_bar_val = lv_bar_get_value(cook->bar_30);
    }

    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_windchange_bbq_complete;
    ...
    if (back) {
        ...
        g_windchange_bbq_stop_back = group_create_for_page(btns, 1);
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_352, tr("保温中..."));
            else
                lv_label_set_text(back->label_352, tr("已完成"));
            lv_bar_set_value(back->bar_32, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_352, tr("预约中..."));
            lv_label_set_text(back->label_354, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_32, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_158, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->button_117, LV_OBJ_FLAG_HIDDEN);
        }

        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_352, tr("烹饪中..."));
    }
    current_group = g_windchange_bbq_stop_back;

    lang_scr_load_anim(windchange_bbq_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[windchange_bbq] jump: stop/cooking -> stop_back\n");
}
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_WINDCHANGE_BBQ_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_WINDCHANGE_BBQ_STOP)
        depth--;
...

    {
        ...
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->button_119, on_windchange_bbq_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            windchange_bbq_set_status(cook->status, set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_33, 100, LV_ANIM_OFF);
        }
    }
...
void windchange_bbq_rebuild_stop_back(void)
{
    ...
    if (back) {
        ...
        if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_WINDCHANGE_BBQ_COOKING) {
        windchange_bbq_cooking_t *cook = windchange_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_30) p = lv_bar_get_value(cook->bar_30);
    }
        if (p <= 0) {
            uint32_t elapsed = cook_timer ? (lv_tick_get() - cook_start_time) : cook_elapsed_saved;
            p = stop_back_progress(elapsed, cook_total_ms);
...
{
    lv_obj_add_event_cb(btn, on_windchange_bbq_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
