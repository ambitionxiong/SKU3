...
void jump_to_west_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->temp, menu->label_146);
        nav_blink_extra(menu->hour, menu->label_149);
        nav_blink_extra(menu->min, menu->label_150);
    ...
    lang_scr_load_anim(west_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        g_send.cook_mode = MODE_COOK4;
...
void jump_to_west_set(void)
{
    ...
    lang_scr_load_anim(west_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_west_cooking(void)
{
    ...
    lang_scr_load_anim(west_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_west_setting(void)
{
    ...
    if (set) {
        ...
        nav_blink_extra(set->hour, set->label_181);
        nav_blink_extra(set->min, set->label_182);
    ...
    lang_scr_load_anim(west_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_west_stop(void)
{
    ...
    lang_scr_load_anim(west_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_188, tr("保温中..."));
            else
                lv_label_set_text(back->label_188, tr("已完成"));
            lv_bar_set_value(back->bar_10, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_188, tr("预约中..."));
            lv_label_set_text(back->label_190, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_188, tr("烹饪中..."));
    }
    current_group = g_west_stop_back;

    lang_scr_load_anim(west_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
