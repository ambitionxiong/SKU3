/*
 * nav_hotcleanhigh.c - 高温清洁(强力档)
 * 高温清洁三档之一：设置/烹饪/暂停/确认退出/冷却/完成。
 */
#include "protocol.h"
#include "nav.h"

static void on_hch_set_start_click(lv_event_t *e);
static void on_hch_cooking_stop_click(lv_event_t *e);
static void on_hch_stop_start_click(lv_event_t *e);
static void on_hch_stop_back_sure_click(lv_event_t *e);
...
void jump_to_hch_set(void)
{
    ...
    hotcleanhigh_set_t *set = hotcleanhigh_set_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->start };
        if (g_hch_set) lv_group_del(g_hch_set);
        g_hch_set = group_create_for_page(btns, 1);

        lv_obj_add_event_cb(set->start, on_hch_set_start_click,
    ...
    g_send.iface_status = IFACE_SETTING;
    printf("[hcs] jump: hotclean_menu -> hch_set\n");
}
...
void jump_to_hch_cooking(void)
{
    ...
    page_push(PAGE_HOTCLEANHIGH_COOKING);
    lv_obj_clean(lv_scr_act());
    ...
    if (cook) {
        ...
        if (g_hch_cooking) lv_group_del(g_hch_cooking);
        g_hch_cooking = group_create_for_page(btns, 1);
    ...
    current_group = g_hch_cooking;

    lang_scr_load_anim(hotcleanhigh_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hch_stop(void)
{
    ...
    page_push(PAGE_HOTCLEANHIGH_STOP);
    lv_obj_clean(lv_scr_act());
    hotcleanhigh_stop_create(&ui_manager);

    hotcleanhigh_stop_t *stop = hotcleanhigh_stop_get(&ui_manager);
    if (stop) {
        lv_obj_t *btns[] = { stop->start };
        if (g_hch_stop) lv_group_del(g_hch_stop);
        g_hch_stop = group_create_for_page(btns, 1);

        lv_obj_add_event_cb(stop->start, on_hch_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    current_group = g_hch_stop;

    lang_scr_load_anim(hotcleanhigh_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hch_stop_back(void)
{
    ...
    if (back) {
        ...
        if (g_hch_stop_back) lv_group_del(g_hch_stop_back);
        g_hch_stop_back = group_create_for_page(btns, 1);

        lv_obj_add_event_cb(back->sure, on_hch_stop_back_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    lang_scr_load_anim(hotcleanhigh_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
{
    edit_clear();
    page_push(PAGE_HOTCLEANHIGH_COOLING);
    lv_obj_clean(lv_scr_act());
    hotcleanhigh_cooling_create(&ui_manager);

    hotcleanhigh_cooling_t *cool = hotcleanhigh_cooling_get(&ui_manager);
    if (cool) {
        lv_bar_set_range(cool->bar_8, 0, 100);
        lv_bar_set_value(cool->bar_8, 100, LV_ANIM_OFF);
    }

    if (g_hch_cooling) lv_group_del(g_hch_cooling);
    {
        lv_obj_t *btns[] = { cool->button_12 };
        g_hch_cooling = group_create_for_page(btns, 1);
    }
    current_group = g_hch_cooling;
    ...
    lang_scr_load_anim(hotcleanhigh_cooling_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    g_send.iface_status = IFACE_COOKING;
    g_send.set_temp = 0;
    printf("[hch] jump: cooking -> cooling END\n");
}

void jump_to_hch_complete(void)
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTCLEANHIGH_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTCLEANHIGH_STOP)
        depth--;
    page_push(PAGE_HOTCLEANHIGH_COMPLETE);
    lv_obj_clean(lv_scr_act());
...
void hch_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (child == PAGE_HOTCLEANHIGH_COOLING || child == PAGE_HOTCLEANHIGH_STOP) {
            uint32_t elapsed = (child == PAGE_HOTCLEANHIGH_COOLING) ? cook_total_ms : (lv_tick_get() - cook_start_time);
    ...
    lang_scr_load_anim(hotcleanhigh_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hcs] back to hch_cooking\n");
}
...
void hch_rebuild_stop_back(void)
{
    ...
    lang_scr_load_anim(hotcleanhigh_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hcs] back to hch_stop_back\n");
}
...
void hch_rebuild_cooling(void)
{
    ...
    lang_scr_load_anim(hotcleanhigh_cooling_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hcs] back to hch_cooling\n");
}
...
void hch_rebuild_complete(void)
{
    ...
    lang_scr_load_anim(hotcleanhigh_complete_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hcs] back to hch_complete\n");
}
