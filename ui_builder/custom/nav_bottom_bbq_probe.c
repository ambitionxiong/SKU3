/*
 * nav_bottom_bbq_probe.c - 底部烧烤探针版
 * 探针模式下底部烧烤全流程，进度按探针目标温度驱动。
 */
#include "protocol.h"
#include "nav.h"
...
static void on_bottom_bbq_probe_set_sure_click(lv_event_t *e);
static void on_bottom_bbq_probe_cooking_stop_click(lv_event_t *e);
static void on_bottom_bbq_probe_stop_start_click(lv_event_t *e);
...
static void on_bottom_bbq_probe_set_sure_click(lv_event_t *e)
{
    ...
    if (!screen_is_loading(act_scr)) {
        if (delay_on)
            ...
            jump_to_bottom_bbq_cooking_probe();
    }
}
...
static void on_bottom_bbq_probe_stop_start_click(lv_event_t *e)
{
    lv_obj_t *act_scr = lv_scr_act();
    if (!screen_is_loading(act_scr))
        bottom_bbq_probe_resume_cooking();
}
...
void jump_to_bottom_bbq_menu_probe(void)
{
    ...
    page_push(PAGE_BOTTOM_BBQ_MENU_PROBE);
    lv_obj_clean(lv_scr_act());
    bottom_bbq_menu_probe_create(&ui_manager);
    ...
    if (menu) {
        ...
        if (g_bottom_bbq_menu_probe) lv_group_del(g_bottom_bbq_menu_probe);
        g_bottom_bbq_menu_probe = group_create_for_page(btns, 3);
        ...
        nav_blink_extra(menu->temp, menu->label_48);
        nav_blink_extra(menu->probetemp, menu->label_53);

        lv_obj_add_event_cb(menu->temp, on_bottom_bbq_probe_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->probetemp, on_bottom_bbq_probe_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_bottom_bbq_probe_edit_focus,
                            LV_EVENT_FOCUSED, NULL);

        if (menu->next)
            lv_obj_add_event_cb(menu->next, on_bottom_bbq_probe_menu_next_click,
    ...
    current_group = g_bottom_bbq_menu_probe;

    lang_scr_load_anim(bottom_bbq_menu_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    g_send.cook_mode = MODE_BOTTOM_BBQ;
    printf("[bottom_bbq_probe] jump: menu_probe\n");
}
...
void jump_to_bottom_bbq_set_probe(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->probetemp, on_bottom_bbq_probe_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_bottom_bbq_probe_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
    ...
    lang_scr_load_anim(bottom_bbq_set_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] jump: set_probe\n");
}
...
void jump_to_bottom_bbq_cooking_probe(void)
{
    ...
    page_push(PAGE_BOTTOM_BBQ_COOKING_PROBE);
    lv_obj_clean(lv_scr_act());
    bottom_bbq_cooking_probe_create(&ui_manager);

    bottom_bbq_cooking_probe_t *cook = bottom_bbq_cooking_probe_get(&ui_manager);
    if (cook) {
        lv_obj_t *btns[] = { cook->stop };
        if (g_bottom_bbq_cooking_probe) lv_group_del(g_bottom_bbq_cooking_probe);
        g_bottom_bbq_cooking_probe = group_create_for_page(btns, 1);
    ...
    current_group = g_bottom_bbq_cooking_probe;

    lang_scr_load_anim(bottom_bbq_cooking_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bottom_bbq_stop_probe(void)
{
    ...
    page_push(PAGE_BOTTOM_BBQ_STOP_PROBE);
    lv_obj_clean(lv_scr_act());
    bottom_bbq_stop_probe_create(&ui_manager);

    bottom_bbq_stop_probe_t *stop = bottom_bbq_stop_probe_get(&ui_manager);
    if (stop) {
        lv_obj_t *btns[] = { stop->start };
        if (g_bottom_bbq_stop_probe) lv_group_del(g_bottom_bbq_stop_probe);
        g_bottom_bbq_stop_probe = group_create_for_page(btns, 1);
        ...
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(stop->bar_6, cook_bar_saved, LV_ANIM_OFF);
    }
    current_group = g_bottom_bbq_stop_probe;

    lang_scr_load_anim(bottom_bbq_stop_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bottom_bbq_stop_back_probe(void)
{
    ...
    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_bottom_bbq_complete_probe;
    page_push(PAGE_BOTTOM_BBQ_STOP_BACK_PROBE);
    lv_obj_clean(lv_scr_act());
    bottom_bbq_stop_back_probe_create(&ui_manager);

    bottom_bbq_stop_back_probe_t *back = bottom_bbq_stop_back_probe_get(&ui_manager);
    if (back) {
        lv_obj_t *btns[] = { back->sure };
        if (g_bottom_bbq_stop_back_probe) lv_group_del(g_bottom_bbq_stop_back_probe);
        g_bottom_bbq_stop_back_probe = group_create_for_page(btns, 1);
        ...
        if (g_delay_cancel_to_stop_back) {
            ...
            lv_label_set_text(back->label_74, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
    ...
    lang_scr_load_anim(bottom_bbq_stop_back_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] jump: stop_back_probe\n");
}

void jump_to_bottom_bbq_complete_probe(void)
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_BOTTOM_BBQ_STOP_BACK_PROBE)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_BOTTOM_BBQ_STOP_PROBE)
        depth--;
    page_push(PAGE_BOTTOM_BBQ_COMPLETE_PROBE);
    lv_obj_clean(lv_scr_act());
    bottom_bbq_complete_probe_create(&ui_manager);

    {
        bottom_bbq_complete_probe_t *complete = bottom_bbq_complete_probe_get(&ui_manager);
        if (complete) {
            lv_obj_t *btns[] = { complete->image_32 };
    ...
    g_send.cook_flag = 0;
    g_send.remaining_ms = 0;
    printf("[bottom_bbq_probe] jump: complete_probe\n");
}
...
void bottom_bbq_probe_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_5, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_5, cook_bar_saved, LV_ANIM_OFF);
    }
...
void bottom_bbq_probe_rebuild_menu(page_id_t child)
{
    ...
    lang_scr_load_anim(bottom_bbq_menu_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] back to bottom_bbq_menu_probe\n");
}
...
void bottom_bbq_probe_rebuild_set(page_id_t child)
{
    ...
    lang_scr_load_anim(bottom_bbq_set_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] back to bottom_bbq_set_probe\n");
}
...
void bottom_bbq_probe_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (bar_val > 100) bar_val = 100;
        lv_bar_set_value(cook->bar_5, bar_val, LV_ANIM_OFF);
    }
    current_group = g_bottom_bbq_cooking_probe;
    lang_scr_load_anim(bottom_bbq_cooking_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] back to bottom_bbq_cooking_probe\n");
}
...
void bottom_bbq_probe_rebuild_stop(void)
{
    ...
    lang_scr_load_anim(bottom_bbq_stop_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] back to bottom_bbq_stop_probe\n");
    g_send.iface_status = IFACE_PAUSE;
}
...
void bottom_bbq_probe_rebuild_stop_back(void)
{
    ...
    lang_scr_load_anim(bottom_bbq_stop_back_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] back to bottom_bbq_stop_back_probe\n");
}
...
void bottom_bbq_probe_rebuild_complete(void)
{
    ...
    lang_scr_load_anim(bottom_bbq_complete_probe_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq_probe] back to bottom_bbq_complete_probe\n");
}

void bottom_bbq_probe_complete_rebind(lv_obj_t *btn)
{
    lv_obj_add_event_cb(btn, on_bottom_bbq_probe_complete_click, LV_EVENT_CLICKED, NULL);
}
