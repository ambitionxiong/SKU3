...
int bottom_bbq_setting_saved_temp, bottom_bbq_setting_saved_hour, bottom_bbq_setting_saved_min;
...
void jump_to_bottom_bbq_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->temp, menu->label_111);
        nav_blink_extra(menu->hour, menu->label_114);
        nav_blink_extra(menu->min, menu->label_115);
    ...
    lang_scr_load_anim(bottom_bbq_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bottom_bbq_set(void)
{
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_flag(set->label_125, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 312, 254);
        lv_obj_set_pos(set->label_126, 365, 269);
    } else {
        lv_obj_clear_flag(set->hour, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(set->label_125, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 395, 254);
        lv_obj_set_pos(set->label_126, 448, 269);
    }
    ...
    lang_scr_load_anim(bottom_bbq_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bottom_bbq_cooking(void)
{
    ...
    lang_scr_load_anim(bottom_bbq_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bottom_bbq_setting(void)
{
    ...
    bottom_bbq_setting_saved_temp = set_temp; bottom_bbq_setting_saved_hour = set_hour; bottom_bbq_setting_saved_min = set_min;
    ...
    if (set) {
        ...
        nav_blink_extra(set->hour, set->label_145);
        nav_blink_extra(set->min, set->label_146);
    ...
    lang_scr_load_anim(bottom_bbq_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bottom_bbq_stop(void)
{
    ...
    lang_scr_load_anim(bottom_bbq_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_bottom_bbq_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_BOTTOM_BBQ_COOKING) {
        bottom_bbq_cooking_t *cook = bottom_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_10) cooking_bar_val = lv_bar_get_value(cook->bar_10);
    }

    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_bottom_bbq_complete;
    ...
    if (back) {
        ...
        g_bottom_bbq_stop_back = group_create_for_page(btns, 1);
        ...
        lv_bar_set_range(back->bar_12, 0, 100);
                lv_bar_set_value(back->bar_12, p, LV_ANIM_OFF);
        ...
        if (g_delay_cancel_to_stop_back) {
            ...
            lv_label_set_text(back->label_154, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
    ...
    lang_scr_load_anim(bottom_bbq_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[bottom_bbq] jump: stop/cooking -> stop_back\n");
}
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_BOTTOM_BBQ_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_BOTTOM_BBQ_STOP)
        depth--;
...
void bottom_bbq_rebuild_stop_back(void)
{
    ...
    if (back) {
        ...
        if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_BOTTOM_BBQ_COOKING) {
        bottom_bbq_cooking_t *cook = bottom_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_10) p = lv_bar_get_value(cook->bar_10);
    }
        if (p <= 0) {
            uint32_t elapsed = cook_timer ? (lv_tick_get() - cook_start_time) : cook_elapsed_saved;
            p = stop_back_progress(elapsed, cook_total_ms);
...
{
    lv_obj_add_event_cb(btn, on_bottom_bbq_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
