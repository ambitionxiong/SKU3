...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 空气炸 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 空气炸 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
static void on_air_stop_back_sure_click(lv_event_t *e)
{
    ...
    set_temp = 230; set_temp_up = 230; set_temp_down = 230; set_hour = 0; set_min = 30;
...
void jump_to_air_set(void)
{
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_flag(set->label_305, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 312 + lang_dyn_dx(), 254 + lang_dyn_dy());
        lv_obj_set_pos(set->label_306, 365 + lang_dyn_dx(), 269 + lang_dyn_dy());
    } else {
        lv_obj_clear_flag(set->hour, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(set->label_305, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 395 + lang_dyn_dx(), 254 + lang_dyn_dy());
        lv_obj_set_pos(set->label_306, 448 + lang_dyn_dx(), 269 + lang_dyn_dy());
    }
...
{
    edit_clear();
    int cooking_bar_val = 0; if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_AIR_COOKING) {
        air_cooking_t *cook = air_cooking_get(&ui_manager);
        if (cook && cook->bar_20) cooking_bar_val = lv_bar_get_value(cook->bar_20);
    }
    g_on_stop_back = 1;
    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_333, tr("保温中..."));
            else
                lv_label_set_text(back->label_333, tr("已完成"));
            lv_bar_set_value(back->bar_22, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_333, tr("预约中..."));
            lv_label_set_text(back->label_335, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_22, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_152, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->little, LV_OBJ_FLAG_HIDDEN);
        }

        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_333, tr("烹饪中..."));
    }
...

    {
        ...
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_air_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, tr("| 空气炸 | %d℃ | %02d分钟"), set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, tr("| 空气炸 | %d℃ | %d小时%02d分钟"), set_temp, set_hour, set_min);
