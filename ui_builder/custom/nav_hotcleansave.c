...
static void on_hcs_set_start_click(lv_event_t *e)
{
    ...
    if (!screen_is_loading(act_scr))
        jump_to_hcs_doorwait();   /* 先进开门检测等待页,关门自动开始(2026-09-24) */
}
...
void jump_to_hcs_set(void)
{
    ...
    if (set) {
        ...
        hotclean_set_align(set->image_9, set->label_14, set->status, set->label_16);   /* 对齐 cooking 基准 */
    }
    current_group = g_hcs_set;

    lang_scr_load_anim(hotcleansave_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hcs_cooking(void)
{
    ...
    lang_scr_load_anim(hotcleansave_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hcs_stop(void)
{
    ...
    lang_scr_load_anim(hotcleansave_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hcs_stop_back(void)
{
    ...
    if (back) {
        ...
        lv_bar_set_value(back->bar_4, p, LV_ANIM_OFF);
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_21, tr("清洁中..."));

        if (g_complete_to_stop_back) {
            g_complete_to_stop_back = 0;
            lv_label_set_text(back->label_21, tr("已完成"));
        ...
        if (g_cooling_to_stop_back) {
            ...
            lv_label_set_text(back->label_21, tr("冷却中..."));
        }
    ...
    lang_scr_load_anim(hotcleansave_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTCLEANSAVE_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOTCLEANSAVE_STOP)
        depth--;
