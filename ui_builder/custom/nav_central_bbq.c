...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 集中烧烤 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 集中烧烤 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
void jump_to_central_bbq_menu(void)
{
    ...
    lang_scr_load_anim(central_bbq_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_central_bbq_set(void)
{
    ...
    if (set_hour == 0) {
        ...
        lv_obj_set_pos(set->label_298, 365 + lang_dyn_dx(), 269 + lang_dyn_dy());
    } else {
        ...
        lv_obj_set_pos(set->label_298, 448 + lang_dyn_dx(), 269 + lang_dyn_dy());
    }
    ...
    lang_scr_load_anim(central_bbq_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_central_bbq_cooking(void)
{
    ...
    lang_scr_load_anim(central_bbq_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_central_bbq_setting(void)
{
    ...
    lang_scr_load_anim(central_bbq_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_central_bbq_stop(void)
{
    ...
    lang_scr_load_anim(central_bbq_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_314, tr("保温中..."));
            else
                lv_label_set_text(back->label_314, tr("已完成"));
            lv_bar_set_value(back->bar_28, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_314, tr("预约中..."));
            lv_label_set_text(back->label_316, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_314, tr("烹饪中..."));
    }
    current_group = g_central_bbq_stop_back;

    lang_scr_load_anim(central_bbq_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
