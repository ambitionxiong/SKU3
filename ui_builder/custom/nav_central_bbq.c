...
static void on_central_bbq_stop_back_littal_click(lv_event_t *e);
...
void jump_to_central_bbq_stop(void)
{
    ...
    {
        central_bbq_cooking_t *cook = central_bbq_cooking_get(&ui_manager);
        cook_bar_saved = cook ? lv_bar_get_value(cook->bar_26) : 0;
    }
...
void jump_to_central_bbq_stop_back(void)
{
    ...
    if (back) {
        lv_obj_t *btns[] = { back->sure, back->button_102 };
        if (g_central_bbq_stop_back) lv_group_del(g_central_bbq_stop_back);
        g_central_bbq_stop_back = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(back->sure, on_central_bbq_stop_back_sure_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(back->button_102, on_central_bbq_stop_back_littal_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        if (_p > 100) _p = 100;
        lv_bar_set_value(back->bar_28, cook_bar_saved, LV_ANIM_OFF);
    }
    ...
    lv_scr_load_anim(central_bbq_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[central_bbq] jump: stop -> stop_back\n");
}
