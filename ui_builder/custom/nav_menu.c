...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, "| 菜单 | %d℃ | %02d分钟", temp, min);
    else
        lv_label_set_text_fmt(label, "| 菜单 | %d℃ | %d小时%02d分钟", temp, hour, min);
}
...

    {
        ...
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_menu_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, "| 菜单 | %d℃ | %02d分钟", set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, "| 菜单 | %d℃ | %d小时%02d分钟", set_temp, set_hour, set_min);
