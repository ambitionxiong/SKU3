...
static void on_menu_cooking_setting_click(lv_event_t *e);
static void on_menu_setting_sure_click(lv_event_t *e);
static void on_menu_stop_start_click(lv_event_t *e);
static void on_menu_stop_back_sure_click(lv_event_t *e);
static void on_menu_edit_focus(lv_event_t *e);
void update_menu_dir_icon(menu_setting_t *set);
static void menu_set_status(lv_obj_t *label, int temp, int hour, int min);
static void menu_preheat_toggle(lv_event_t *e);
static void menu_delay_toggle(lv_event_t *e);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 菜单 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 菜单 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
static void on_menu_stop_back_sure_click(lv_event_t *e)
{
    ...
    printf("[menu] stop_back sure -> major_menu\n");
}
...
{
    set_temp = 190; set_hour = 0; set_min = 30;
    page_push(PAGE_MENU_COOK_MENU);
    lv_obj_clean(lv_scr_act());
    menu_menu_create(&ui_manager);
    ...
    if (menu) {
        ...
        if (g_menu_cook_menu) lv_group_del(g_menu_cook_menu);
        ...
        nav_blink_extra(menu->temp, menu->label_243);
        nav_blink_extra(menu->hour, menu->label_246);
        nav_blink_extra(menu->min, menu->label_247);

        lv_obj_add_event_cb(menu->temp, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->hour, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
    ...
    current_group = g_menu_cook_menu;

    lang_scr_load_anim(menu_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        g_send.cook_mode = MODE_COOK4;
    printf("[menu] jump: cookmenu -> menu_menu\n");
}

// menu → set
void jump_to_menu_set(void)
{
    g_delay_source_page = PAGE_MENU_COOK_SET;   /* 记录来源子类：收藏卡片名/启动分发依赖 */
    page_push(PAGE_MENU_COOK_SET);
    lv_obj_clean(lv_scr_act());
    menu_set_create(&ui_manager);
    ...
    lang_scr_load_anim(menu_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[menu] jump: menu -> menu_set\n");
}
...
void jump_to_menu_cooking(void)
{
    ...
    if (cook) {
        ...
        if (g_menu_cook_cooking) lv_group_del(g_menu_cook_cooking);
        g_menu_cook_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_menu_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->little, on_menu_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        menu_set_status(cook->status, set_temp, set_hour, set_min);
        lv_label_set_text_fmt(cook->timelabel, "%02d:%02d:%02d", set_hour, set_min, 0);
    }

    cook_total_ms = (set_hour * 3600 + set_min * 60) * 1000;
    if (cook) {
        lv_bar_set_range(cook->bar_16, 0, 100);
        lv_bar_set_value(cook->bar_16, 3, LV_ANIM_OFF);

        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, cook->bar_16);
    ...
    current_group = g_menu_cook_cooking;

    lang_scr_load_anim(menu_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[menu] jump: set -> menu_cooking\n");
}
...
void jump_to_menu_setting(void)
{
    ...
    if (set) {
        ...
        if (g_menu_cook_setting) lv_group_del(g_menu_cook_setting);
        ...
        nav_blink_extra(set->hour, set->label_277);
        nav_blink_extra(set->min, set->label_278);

        lv_obj_add_event_cb(set->temp, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_menu_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_menu_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    current_group = g_menu_cook_setting;

    lang_scr_load_anim(menu_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_menu_stop(void)
{
    ...
    page_push(PAGE_MENU_COOK_STOP);
    lv_obj_clean(lv_scr_act());
    menu_stop_create(&ui_manager);

    menu_stop_t *stop = menu_stop_get(&ui_manager);
    if (stop) {
        lv_obj_t *btns[] = { stop->start, stop->little };
        if (g_menu_cook_stop) lv_group_del(g_menu_cook_stop);
        g_menu_cook_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_menu_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_menu_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        menu_set_status(stop->status, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_17, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(stop->bar_17, cook_bar_saved, LV_ANIM_OFF);
    }
    current_group = g_menu_cook_stop;

    lang_scr_load_anim(menu_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[menu] jump: cooking -> stop (pause)\n");
}
...
void jump_to_menu_stop_back(void)
{
    ...
    if (back) {
        ...
        menu_set_status(back->status, set_temp, set_hour, set_min);
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_284, tr("保温中..."));
            else
                lv_label_set_text(back->label_284, tr("已完成"));
            lv_bar_set_value(back->bar_18, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_284, tr("预约中..."));
            lv_label_set_text(back->label_286, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_284, tr("烹饪中..."));
    }
    current_group = g_menu_cook_stop_back;

    lang_scr_load_anim(menu_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[menu] jump: stop/cooking -> stop_back\n");
}
...
void menu_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_16, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_16, cook_bar_saved, LV_ANIM_OFF);
    ...
    printf("[menu] resume: stop -> cooking\n");
}
...
static void on_menu_setting_sure_click(lv_event_t *e)
{
    ...
    if (depth > 1) depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_MENU_COOK_STOP)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_MENU_COOK_COMPLETE)
        depth--;
    lv_obj_clean(lv_scr_act());
    menu_cooking_create(&ui_manager);
    ...
    g_send.set_temp = set_temp;
    g_send.remaining_ms = cook_total_ms;
    printf("[menu] setting sure -> cooking\n");
}
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_MENU_COOK_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_MENU_COOK_STOP)
        depth--;
    page_push(PAGE_MENU_COOK_COMPLETE);
    lv_obj_clean(lv_scr_act());
    menu_complete_create(&ui_manager);

    {
        menu_complete_t *cook = menu_complete_get(&ui_manager);
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_menu_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, tr("| 菜单 | %d℃ | %02d分钟"), set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, tr("| 菜单 | %d℃ | %d小时%02d分钟"), set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_19, 100, LV_ANIM_OFF);
        }
    }
...
void menu_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (child == PAGE_MENU_COOK_SETTING) {
            ...
            lv_bar_set_value(cook->bar_16, progress, LV_ANIM_OFF);
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, cook->bar_16);
            lv_anim_set_exec_cb(&a, anim_bar_set_value);
    ...
    lang_scr_load_anim(menu_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[menu] back to menu_cooking\n");
}
...
void menu_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->temp, on_menu_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_menu_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_menu_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_menu_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_menu_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    lang_scr_load_anim(menu_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[menu] back to menu_setting\n");
}
...
void menu_rebuild_stop_back(void)
{
    ...
    if (back) {
        ...
        menu_set_status(back->status, set_temp, set_hour, set_min);
        lv_bar_set_range(back->bar_18, 0, 100);
        uint32_t elapsed = cook_timer ? (lv_tick_get() - cook_start_time) : cook_elapsed_saved;
    ...
    lang_scr_load_anim(menu_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[menu] back to menu_stop_back\n");
}
...
void menu_rebuild_complete(void)
{
    ...
    lang_scr_load_anim(menu_complete_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[menu] back to menu_complete\n");
}
void menu_complete_rebind(lv_obj_t *btn)
{
    lv_obj_add_event_cb(btn, on_menu_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
