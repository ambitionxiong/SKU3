    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_200, "保温中...");
            else
                lv_label_set_text(back->label_200, "已完成");
            lv_bar_set_value(back->bar_16, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_200, "预约中...");
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_200, "烹饪中...");
    }
...

    {
        ...
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->button_59, on_hot_bbq_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, "| 热风烧烤 | %d℃ | %02d分钟", set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, "| 热风烧烤 | %d℃ | %d小时%02d分钟", set_temp, set_hour, set_min);
