...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 热风烧烤 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 热风烧烤 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
void jump_to_hot_bbq_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->temp, menu->label_159);
        nav_blink_extra(menu->hour, menu->label_162);
        nav_blink_extra(menu->min, menu->label_163);
    ...
    lang_scr_load_anim(hot_bbq_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_set(void)
{
    ...
    lang_scr_load_anim(hot_bbq_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_cooking(void)
{
    ...
    lang_scr_load_anim(hot_bbq_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_setting(void)
{
    ...
    if (set) {
        ...
        nav_blink_extra(set->hour, set->label_193);
        nav_blink_extra(set->min, set->label_194);
    ...
    lang_scr_load_anim(hot_bbq_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_stop(void)
{
    ...
    lang_scr_load_anim(hot_bbq_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_hot_bbq_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_HOT_BBQ_COOKING) {
        hot_bbq_cooking_t *cook = hot_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_14) cooking_bar_val = lv_bar_get_value(cook->bar_14);
    }

    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_hot_bbq_complete;
    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_200, tr("保温中..."));
            else
                lv_label_set_text(back->label_200, tr("已完成"));
            lv_bar_set_value(back->bar_16, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_200, tr("预约中..."));
            lv_label_set_text(back->label_202, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_16, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_94, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->button_57, LV_OBJ_FLAG_HIDDEN);
        }

        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_200, tr("烹饪中..."));
    }
    current_group = g_hot_bbq_stop_back;

    lang_scr_load_anim(hot_bbq_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[hot_bbq] jump: stop/cooking -> stop_back\n");
}
...
static void on_hot_bbq_setting_sure_click(lv_event_t *e)
{
    ...
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOT_BBQ_STOP)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOT_BBQ_COMPLETE)
        depth--;
    lv_obj_clean(lv_scr_act());
    hot_bbq_cooking_create(&ui_manager);
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOT_BBQ_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_HOT_BBQ_STOP)
        depth--;
    ...
    {
        hot_bbq_complete_t *cook = hot_bbq_complete_get(&ui_manager);
        if (cook) {
            lv_obj_t *btns[] = { cook->button_59 };
            if (g_hot_bbq_complete) lv_group_del(g_hot_bbq_complete);
            g_hot_bbq_complete = group_create_for_page(btns, 1);
            lv_obj_add_event_cb(cook->button_59, on_hot_bbq_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, tr("| 热风烧烤 | %d℃ | %02d分钟"), set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, tr("| 热风烧烤 | %d℃ | %d小时%02d分钟"), set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_17, 100, LV_ANIM_OFF);
        }
    }
...
{
    lv_obj_add_event_cb(btn, on_hot_bbq_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
