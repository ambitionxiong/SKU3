...
int slowcook_setting_saved_temp, slowcook_setting_saved_hour, slowcook_setting_saved_min;


static void on_slowcook_menu_next_click(lv_event_t *e);
static void on_slowcook_set_sure_click(lv_event_t *e);
static void on_slowcook_cooking_stop_click(lv_event_t *e);
static void on_slowcook_cooking_setting_click(lv_event_t *e);
static void on_slowcook_setting_sure_click(lv_event_t *e);
static void on_slowcook_stop_start_click(lv_event_t *e);
static void on_slowcook_stop_back_sure_click(lv_event_t *e);
static void on_slowcook_edit_focus(lv_event_t *e);
void update_slowcook_dir_icon(slowcook_setting_t *set);
static void slowcook_set_status(lv_obj_t *label, int temp, int hour, int min);
static void slowcook_preheat_toggle(lv_event_t *e);
static void slowcook_delay_toggle(lv_event_t *e);
static void slowcook_contain_toggle(lv_event_t *e);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 慢煮 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 慢煮 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}

static void slowcook_preheat_toggle(lv_event_t *e)
{
    slowcook_set_t *set = slowcook_set_get(&ui_manager);
    if (!set) return;
...
static void on_slowcook_stop_back_sure_click(lv_event_t *e)
{
    ...
    set_temp = 80; set_temp_up = 80; set_temp_down = 80; set_hour = 0; set_min = 30;
...
{
    set_temp = 80; set_hour = 0; set_min = 30;
    page_push(PAGE_SLOWCOOK_MENU);
    lv_obj_clean(lv_scr_act());
    slowcook_menu_create(&ui_manager);
    ...
    if (menu) {
        ...
        if (g_slowcook_menu) lv_group_del(g_slowcook_menu);
        ...
        nav_blink_extra(menu->temp, menu->label_389);
        nav_blink_extra(menu->hour, menu->label_392);
        nav_blink_extra(menu->min, menu->label_393);

        lv_obj_add_event_cb(menu->temp, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->hour, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
    ...
    current_group = g_slowcook_menu;

    lang_scr_load_anim(slowcook_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
{
    page_push(PAGE_SLOWCOOK_SET);
    lv_obj_clean(lv_scr_act());
    slowcook_set_create(&ui_manager);
    ...
    if (set) {
        ...
        if (g_slowcook_set) lv_group_del(g_slowcook_set);
        g_slowcook_set = group_create_for_page(btns, 7);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_flag(set->label_403, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 312, 254);
        lv_obj_set_pos(set->label_404, 365, 269);
    } else {
        lv_obj_clear_flag(set->hour, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(set->label_403, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 395, 254);
        lv_obj_set_pos(set->label_404, 448, 269);
    }
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offpreheat, slowcook_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->onpreheat, slowcook_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offdelay, slowcook_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, slowcook_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offcontain, slowcook_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->oncontain, slowcook_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->sure, on_slowcook_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
    }
    current_group = g_slowcook_set;

    lang_scr_load_anim(slowcook_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_slowcook_cooking(void)
{
    ...
    page_push(PAGE_SLOWCOOK_COOKING);
    lv_obj_clean(lv_scr_act());
    ...
    if (cook) {
        ...
        if (g_slowcook_cooking) lv_group_del(g_slowcook_cooking);
        g_slowcook_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_slowcook_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->little, on_slowcook_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        slowcook_set_status(cook->status, set_temp, set_hour, set_min);
        lv_label_set_text_fmt(cook->timelabel, "%02d:%02d:%02d", set_hour, set_min, 0);
    }

    cook_total_ms = (set_hour * 3600 + set_min * 60) * 1000;
    if (cook) {
        lv_bar_set_range(cook->bar_28, 0, 100);
        lv_bar_set_value(cook->bar_28, 3, LV_ANIM_OFF);

        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, cook->bar_28);
    ...
    current_group = g_slowcook_cooking;

    lang_scr_load_anim(slowcook_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
{
    slowcook_setting_saved_temp = set_temp; slowcook_setting_saved_hour = set_hour; slowcook_setting_saved_min = set_min;
    page_push(PAGE_SLOWCOOK_SETTING);
    lv_obj_clean(lv_scr_act());
    slowcook_setting_create(&ui_manager);

    slowcook_setting_t *set = slowcook_setting_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->temp, set->hour, set->min, set->sure };
        if (g_slowcook_setting) lv_group_del(g_slowcook_setting);
        ...
        nav_blink_extra(set->hour, set->label_424);
        nav_blink_extra(set->min, set->label_425);

        lv_obj_add_event_cb(set->temp, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_slowcook_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_slowcook_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        lv_obj_add_flag(set->icon2, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(set->houeline, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(set->minline, LV_OBJ_FLAG_HIDDEN);

        lv_group_focus_obj(set->temp);
        update_slowcook_dir_icon(set);
    }
    current_group = g_slowcook_setting;

    lang_scr_load_anim(slowcook_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_slowcook_stop(void)
{
    ...
    page_push(PAGE_SLOWCOOK_STOP);
    lv_obj_clean(lv_scr_act());
    slowcook_stop_create(&ui_manager);

    slowcook_stop_t *stop = slowcook_stop_get(&ui_manager);
    if (stop) {
        lv_obj_t *btns[] = { stop->start, stop->little };
        if (g_slowcook_stop) lv_group_del(g_slowcook_stop);
        g_slowcook_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_slowcook_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_slowcook_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        slowcook_set_status(stop->status, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_29, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(stop->bar_29, cook_bar_saved, LV_ANIM_OFF);
    }
    current_group = g_slowcook_stop;

    lang_scr_load_anim(slowcook_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_slowcook_stop_back(void)
{
    ...
    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_slowcook_complete;
    page_push(PAGE_SLOWCOOK_STOP_BACK);
    lv_obj_clean(lv_scr_act());
    ...
    if (back) {
        ...
        slowcook_set_status(back->status, set_temp, set_hour, set_min);
        int cooking_bar_val = 0;
        if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_SLOWCOOK_COOKING) {
        slowcook_cooking_t *cook = slowcook_cooking_get(&ui_manager);
        if (cook && cook->bar_28) cooking_bar_val = lv_bar_get_value(cook->bar_28);
    }
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_431, tr("保温中..."));
            else
                lv_label_set_text(back->label_431, tr("已完成"));
            lv_bar_set_value(back->bar_30, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_431, tr("预约中..."));
            lv_label_set_text(back->label_433, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_30, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_194, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->little, LV_OBJ_FLAG_HIDDEN);
        }

        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_431, tr("烹饪中..."));
    }
    current_group = g_slowcook_stop_back;

    lang_scr_load_anim(slowcook_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void slowcook_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_28, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_28, cook_bar_saved, LV_ANIM_OFF);
...
static void on_slowcook_setting_sure_click(lv_event_t *e)
{
    ...
    if (depth > 1) depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_SLOWCOOK_STOP)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_SLOWCOOK_COMPLETE)
        depth--;
    lv_obj_clean(lv_scr_act());
    slowcook_cooking_create(&ui_manager);
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_SLOWCOOK_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_SLOWCOOK_STOP)
        depth--;
    page_push(PAGE_SLOWCOOK_COMPLETE);
    lv_obj_clean(lv_scr_act());
    slowcook_complete_create(&ui_manager);

    {
        slowcook_complete_t *cook = slowcook_complete_get(&ui_manager);
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_slowcook_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, tr("| 慢煮 | %d℃ | %02d分钟"), set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, tr("| 慢煮 | %d℃ | %d小时%02d分钟"), set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_31, 100, LV_ANIM_OFF);
        }
    }
...
void slowcook_rebuild_set(page_id_t child)
{
    ...
    if (set_hour == 0) {
        ...
        if (child == PAGE_SLOWCOOK_COOKING && set->sure)
            lv_group_focus_obj(set->sure);
    }
...
void slowcook_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (child == PAGE_SLOWCOOK_SETTING) {
            ...
            lv_bar_set_value(cook->bar_28, progress, LV_ANIM_OFF);
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, cook->bar_28);
            lv_anim_set_exec_cb(&a, anim_bar_set_value);
...
void slowcook_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->temp, on_slowcook_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_slowcook_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_slowcook_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_slowcook_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_slowcook_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
...
{
    lv_obj_add_event_cb(btn, on_slowcook_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
