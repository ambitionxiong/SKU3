...
int rising_setting_saved_temp, rising_setting_saved_hour, rising_setting_saved_min;


static void on_rising_menu_next_click(lv_event_t *e);
static void on_rising_set_sure_click(lv_event_t *e);
static void on_rising_cooking_stop_click(lv_event_t *e);
static void on_rising_cooking_setting_click(lv_event_t *e);
static void on_rising_setting_sure_click(lv_event_t *e);
static void on_rising_stop_start_click(lv_event_t *e);
static void on_rising_stop_back_sure_click(lv_event_t *e);
static void on_rising_edit_focus(lv_event_t *e);
void update_rising_dir_icon(rising_setting_t *set);
static void rising_set_status(lv_obj_t *label, int temp, int hour, int min);
static void rising_preheat_toggle(lv_event_t *e);
static void rising_delay_toggle(lv_event_t *e);
static void rising_contain_toggle(lv_event_t *e);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 发酵 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 发酵 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
static void on_rising_stop_back_sure_click(lv_event_t *e)
{
    ...
    set_temp = 35; set_temp_up = 35; set_temp_down = 35; set_hour = 0; set_min = 30;
    ...
    printf("[rising] stop_back sure -> major_menu\n");
}
...
{
    set_temp = 35; set_hour = 0; set_min = 30;
    page_push(PAGE_RISING_MENU);
    lv_obj_clean(lv_scr_act());
    rising_menu_create(&ui_manager);
    ...
    if (menu) {
        ...
        if (g_rising_menu) lv_group_del(g_rising_menu);
        g_rising_menu = group_create_for_page(btns, 4);
        ...
        lv_obj_add_event_cb(menu->hour, on_rising_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min, on_rising_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_rising_edit_focus,
                            LV_EVENT_FOCUSED, NULL);

        if (menu->next)
            lv_obj_add_event_cb(menu->next, on_rising_menu_next_click,
    ...
    current_group = g_rising_menu;

    lang_scr_load_anim(rising_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        g_send.cook_mode = MODE_RISING;
    printf("[rising] jump: special_menu -> rising_menu\n");
}

// menu → set
void jump_to_rising_set(void)
{
    page_push(PAGE_RISING_SET);
    lv_obj_clean(lv_scr_act());
    ...
    if (set) {
        ...
        if (g_rising_set) lv_group_del(g_rising_set);
        g_rising_set = group_create_for_page(btns, 7);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_flag(set->label_501, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 312 + lang_dyn_dx(), 254 + lang_dyn_dy());
        lv_obj_set_pos(set->label_502, 365 + lang_dyn_dx(), 269 + lang_dyn_dy());
    } else {
        lv_obj_clear_flag(set->hour, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(set->label_501, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 395 + lang_dyn_dx(), 254 + lang_dyn_dy());
        lv_obj_set_pos(set->label_502, 448 + lang_dyn_dx(), 269 + lang_dyn_dy());
    }
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offpreheat, rising_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->onpreheat, rising_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offdelay, rising_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, rising_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offcontain, rising_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->oncontain, rising_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->sure, on_rising_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
    }
    current_group = g_rising_set;

    lang_scr_load_anim(rising_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] jump: menu -> rising_set\n");
}
...
void jump_to_rising_cooking(void)
{
    ...
    page_push(PAGE_RISING_COOKING);
    lv_obj_clean(lv_scr_act());
    rising_cooking_create(&ui_manager);

    rising_cooking_t *cook = rising_cooking_get(&ui_manager);
    if (cook) {
        lv_obj_t *btns[] = { cook->stop, cook->liitle };
        if (g_rising_cooking) lv_group_del(g_rising_cooking);
        g_rising_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_rising_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->liitle, on_rising_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        rising_set_status(cook->status, set_temp, set_hour, set_min);
        lv_label_set_text_fmt(cook->timelabel, "%02d:%02d:%02d", set_hour, set_min, 0);
    }

    cook_total_ms = (set_hour * 3600 + set_min * 60) * 1000;
    if (cook) {
        lv_bar_set_range(cook->bar_36, 0, 100);
        lv_bar_set_value(cook->bar_36, 3, LV_ANIM_OFF);

        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, cook->bar_36);
    ...
    current_group = g_rising_cooking;

    lang_scr_load_anim(rising_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[rising] jump: set -> rising_cooking\n");
}
...
void jump_to_rising_setting(void)
{
    ...
    lv_obj_clean(lv_scr_act());
    rising_setting_create(&ui_manager);

    rising_setting_t *set = rising_setting_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->temp, set->hour, set->min, set->sure };
        if (g_rising_setting) lv_group_del(g_rising_setting);
        g_rising_setting = group_create_for_page(btns, 4);
        ...
        lv_obj_add_event_cb(set->temp, on_rising_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_rising_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_rising_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_rising_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_rising_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    current_group = g_rising_setting;

    lang_scr_load_anim(rising_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        if (g_send.iface_status != IFACE_COMPLETE)
        g_send.iface_status = (cook_timer != NULL) ? IFACE_COOKING : IFACE_SETTING;
    printf("[rising] jump: cooking -> rising_setting\n");
}
...
void jump_to_rising_stop(void)
{
    ...
    page_push(PAGE_RISING_STOP);
    lv_obj_clean(lv_scr_act());
    rising_stop_create(&ui_manager);

    rising_stop_t *stop = rising_stop_get(&ui_manager);
    if (stop) {
        lv_obj_t *btns[] = { stop->start, stop->little };
        if (g_rising_stop) lv_group_del(g_rising_stop);
        g_rising_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_rising_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_rising_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        rising_set_status(stop->status, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_37, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(stop->bar_37, cook_bar_saved, LV_ANIM_OFF);
    }
    current_group = g_rising_stop;

    lang_scr_load_anim(rising_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[rising] jump: cooking -> stop (pause)\n");
}
...
void jump_to_rising_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_RISING_COOKING) {
        rising_cooking_t *cook = rising_cooking_get(&ui_manager);
        if (cook && cook->bar_36) cooking_bar_val = lv_bar_get_value(cook->bar_36);
    }

    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_rising_complete;
    page_push(PAGE_RISING_STOP_BACK);
    lv_obj_clean(lv_scr_act());
    rising_stop_back_create(&ui_manager);

    rising_stop_back_t *back = rising_stop_back_get(&ui_manager);
    if (back) {
        lv_obj_t *btns[] = { back->sure };
        if (g_rising_stop_back) lv_group_del(g_rising_stop_back);
        g_rising_stop_back = group_create_for_page(btns, 1);
        lv_obj_add_event_cb(back->sure, on_rising_stop_back_sure_click,
                            LV_EVENT_CLICKED, NULL);

        rising_set_status(back->status, set_temp, set_hour, set_min);
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_529, tr("保温中..."));
            else
                lv_label_set_text(back->label_529, tr("已完成"));
            lv_bar_set_value(back->bar_38, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_529, tr("预约中..."));
            lv_label_set_text(back->label_531, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_38, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_236, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->little, LV_OBJ_FLAG_HIDDEN);
        }

        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_529, tr("烹饪中..."));
    }
    current_group = g_rising_stop_back;

    lang_scr_load_anim(rising_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] jump: stop/cooking -> stop_back\n");
}
...
void rising_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_36, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_36, cook_bar_saved, LV_ANIM_OFF);
    ...
    printf("[rising] resume: stop -> cooking\n");
}
...
static void on_rising_setting_sure_click(lv_event_t *e)
{
    ...
    if (depth > 1) depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_RISING_STOP)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_RISING_COMPLETE)
        depth--;
    lv_obj_clean(lv_scr_act());
    rising_cooking_create(&ui_manager);
    ...
    g_send.set_temp = set_temp;
    g_send.remaining_ms = cook_total_ms;
    printf("[rising] setting sure -> cooking\n");
}
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_RISING_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_RISING_STOP)
        depth--;
    page_push(PAGE_RISING_COMPLETE);
    lv_obj_clean(lv_scr_act());
    rising_complete_create(&ui_manager);

    {
        rising_complete_t *cook = rising_complete_get(&ui_manager);
        if (cook) {
            ...
            lv_obj_add_event_cb(cook->little, on_rising_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->status, tr("| 发酵 | %d℃ | %02d分钟"), set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->status, tr("| 发酵 | %d℃ | %d小时%02d分钟"), set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_39, 100, LV_ANIM_OFF);
        }
    }
    ...
    printf("[rising] jump: cooking -> complete\n");
...
void rising_rebuild_menu(page_id_t child)
{
    ...
    lang_scr_load_anim(rising_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] back to rising_menu\n");
}
...
void rising_rebuild_set(page_id_t child)
{
    ...
    if (set_hour == 0) {
        ...
        if (child == PAGE_RISING_COOKING && set->sure)
            lv_group_focus_obj(set->sure);
    }
    ...
    lang_scr_load_anim(rising_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] back to rising_set\n");
}
...
void rising_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (child == PAGE_RISING_SETTING) {
            ...
            lv_bar_set_value(cook->bar_36, progress, LV_ANIM_OFF);
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, cook->bar_36);
            lv_anim_set_exec_cb(&a, anim_bar_set_value);
    ...
    lang_scr_load_anim(rising_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] back to rising_cooking\n");
}
...
void rising_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->min, on_rising_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_rising_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_rising_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    lang_scr_load_anim(rising_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] back to rising_setting\n");
}
...
void rising_rebuild_stop(void)
{
    ...
    lang_scr_load_anim(rising_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] back to rising_stop\n");
    g_send.iface_status = IFACE_PAUSE;
}
...
void rising_rebuild_stop_back(void)
{
    ...
    lang_scr_load_anim(rising_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] back to rising_stop_back\n");
}
...
void rising_rebuild_complete(void)
{
    ...
    lang_scr_load_anim(rising_complete_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[rising] back to rising_complete\n");
}
void rising_complete_rebind(lv_obj_t *btn)
{
    lv_obj_add_event_cb(btn, on_rising_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
