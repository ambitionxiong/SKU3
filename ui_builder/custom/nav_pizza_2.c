...
int pizza_2_setting_saved_temp, pizza_2_setting_saved_hour, pizza_2_setting_saved_min;


static void on_pizza_2_menu_next_click(lv_event_t *e);
static void on_pizza_2_set_sure_click(lv_event_t *e);
static void on_pizza_2_cooking_stop_click(lv_event_t *e);
...
static void pizza_2_set_status(lv_obj_t *label, int temp, int hour, int min);
static void pizza_2_preheat_toggle(lv_event_t *e);
static void pizza_2_delay_toggle(lv_event_t *e);
static void pizza_2_contain_toggle(lv_event_t *e);
...
void jump_to_pizza_2_menu(void)
{
    ...
    page_push(PAGE_PIZZA_2_MENU);
    lv_obj_clean(lv_scr_act());
    pizza_2_menu_create(&ui_manager);
    ...
    if (menu) {
        ...
        if (g_pizza_2_menu) lv_group_del(g_pizza_2_menu);
        g_pizza_2_menu = group_create_for_page(btns, 4);
        ...
        nav_blink_extra(menu->temp, menu->label_340);
        nav_blink_extra(menu->hour, menu->label_343);
        nav_blink_extra(menu->min, menu->label_344);

        lv_obj_add_event_cb(menu->temp, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->hour, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);

        if (menu->next)
            lv_obj_add_event_cb(menu->next, on_pizza_2_menu_next_click,
    ...
    current_group = g_pizza_2_menu;

    lang_scr_load_anim(pizza_2_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        g_send.cook_mode = MODE_PIZZA_2;
    printf("[pizza_2] jump: special_menu -> pizza_2_menu\n");
}

// menu → set
void jump_to_pizza_2_set(void)
{
    page_push(PAGE_PIZZA_2_SET);
    lv_obj_clean(lv_scr_act());
    ...
    if (set) {
        ...
        if (g_pizza_2_set) lv_group_del(g_pizza_2_set);
        g_pizza_2_set = group_create_for_page(btns, 7);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_flag(set->label_354, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 312, 254);
        lv_obj_set_pos(set->label_355, 365, 269);
    } else {
        lv_obj_clear_flag(set->hour, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(set->label_354, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_pos(set->min, 395, 254);
        lv_obj_set_pos(set->label_355, 448, 269);
    }
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offpreheat, pizza_2_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->onpreheat, pizza_2_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offdelay, pizza_2_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, pizza_2_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offcontain, pizza_2_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->oncontain, pizza_2_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->sure, on_pizza_2_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
    }
    current_group = g_pizza_2_set;

    lang_scr_load_anim(pizza_2_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] jump: menu -> pizza_2_set\n");
}
...
void jump_to_pizza_2_cooking(void)
{
    ...
    page_push(PAGE_PIZZA_2_COOKING);
    lv_obj_clean(lv_scr_act());
    ...
    if (cook) {
        ...
        if (g_pizza_2_cooking) lv_group_del(g_pizza_2_cooking);
        g_pizza_2_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_pizza_2_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->little, on_pizza_2_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        pizza_2_set_status(cook->status, set_temp, set_hour, set_min);
    ...
    if (cook) {
        ...
        lv_bar_set_value(cook->bar_24, 3, LV_ANIM_OFF);

        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, cook->bar_24);
    ...
    current_group = g_pizza_2_cooking;

    lang_scr_load_anim(pizza_2_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[pizza_2] jump: set -> pizza_2_cooking\n");
}
...
void jump_to_pizza_2_setting(void)
{
    ...
    pizza_2_setting_saved_temp = set_temp; pizza_2_setting_saved_hour = set_hour; pizza_2_setting_saved_min = set_min;
    page_push(PAGE_PIZZA_2_SETTING);
    lv_obj_clean(lv_scr_act());
    pizza_2_setting_create(&ui_manager);

    pizza_2_setting_t *set = pizza_2_setting_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->temp, set->hour, set->min, set->sure };
        if (g_pizza_2_setting) lv_group_del(g_pizza_2_setting);
        g_pizza_2_setting = group_create_for_page(btns, 4);
        ...
        nav_blink_extra(set->hour, set->label_375);
        nav_blink_extra(set->min, set->label_376);

        lv_obj_add_event_cb(set->temp, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_pizza_2_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_pizza_2_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    current_group = g_pizza_2_setting;

    lang_scr_load_anim(pizza_2_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
        if (g_send.iface_status != IFACE_COMPLETE)
        g_send.iface_status = (cook_timer != NULL) ? IFACE_COOKING : IFACE_SETTING;
    printf("[pizza_2] jump: cooking -> pizza_2_setting\n");
}
...
void jump_to_pizza_2_stop(void)
{
    ...
    if (stop) {
        ...
        if (g_pizza_2_stop) lv_group_del(g_pizza_2_stop);
        g_pizza_2_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_pizza_2_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_pizza_2_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        pizza_2_set_status(stop->status, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_25, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(stop->bar_25, cook_bar_saved, LV_ANIM_OFF);
    }
    current_group = g_pizza_2_stop;

    lang_scr_load_anim(pizza_2_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void jump_to_pizza_2_stop_back(void)
{
    ...
    page_push(PAGE_PIZZA_2_STOP_BACK);
    lv_obj_clean(lv_scr_act());
    ...
    if (back) {
        ...
        if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_PIZZA_2_COOKING) {
        pizza_2_cooking_t *cook = pizza_2_cooking_get(&ui_manager);
        if (cook && cook->bar_24) cooking_bar_val = lv_bar_get_value(cook->bar_24);
    }
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_382, tr("保温中..."));
            else
                lv_label_set_text(back->label_382, tr("已完成"));
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_382, tr("烹饪中..."));
    }
    current_group = g_pizza_2_stop_back;

    lang_scr_load_anim(pizza_2_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
...
void pizza_2_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_24, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_24, cook_bar_saved, LV_ANIM_OFF);
...
void jump_to_pizza_2_complete(void)
{
    ...
    {
        pizza_2_complete_t *cook = pizza_2_complete_get(&ui_manager);
        if (cook) {
...
void pizza_2_rebuild_menu(page_id_t child)
{
    ...
    lang_scr_load_anim(pizza_2_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to pizza_2_menu\n");
}
...
void pizza_2_rebuild_set(page_id_t child)
{
    ...
    if (set_hour == 0) {
        ...
        if (child == PAGE_PIZZA_2_COOKING && set->sure)
            lv_group_focus_obj(set->sure);
    }
    ...
    lang_scr_load_anim(pizza_2_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to pizza_2_set\n");
}
...
void pizza_2_rebuild_cooking(page_id_t child)
{
    ...
    lang_scr_load_anim(pizza_2_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to pizza_2_cooking\n");
}
...
void pizza_2_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->temp, on_pizza_2_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_pizza_2_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_pizza_2_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_pizza_2_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_pizza_2_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    lang_scr_load_anim(pizza_2_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to pizza_2_setting\n");
}
...
void pizza_2_rebuild_stop(void)
{
    ...
    lang_scr_load_anim(pizza_2_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to pizza_2_stop\n");
    g_send.iface_status = IFACE_PAUSE;
}
...
void pizza_2_rebuild_stop_back(void)
{
    ...
    lang_scr_load_anim(pizza_2_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza_2] back to pizza_2_stop_back\n");
}
