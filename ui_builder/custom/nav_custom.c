...
static void on_custom_stop_back_littal_click(lv_event_t *e);
static void on_custom_stop_back_sure_click(lv_event_t *e);
static void on_custom_edit_focus(lv_event_t *e);
static void custom_set_status(lv_obj_t *label, int hour, int min);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, "| 自定义 | %02d分钟", min);
    else
        lv_label_set_text_fmt(label, "| 自定义 | %d小时%02d分钟", hour, min);
}
...
void jump_to_custom_cooking(void)
{
    ...
    if (cook) {
        ...
        custom_set_status(cook->status, set_hour, set_min);
...
void jump_to_custom_stop(void)
{
    ...
    {
        custom_cooking_t *cook = custom_cooking_get(&ui_manager);
        cook_bar_saved = cook ? lv_bar_get_value(cook->bar_68) : 0;
    }
    ...
    if (stop) {
        ...
        custom_set_status(stop->status, set_hour, set_min);
...
void jump_to_custom_stop_back(void)
{
    ...
    if (back) {
        ...
        g_custom_stop_back = group_create_for_page(btns, 2);
        ...
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(back->bar_70, cook_bar_saved, LV_ANIM_OFF);
    }
