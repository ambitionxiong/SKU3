...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 自定义 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 自定义 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
static void on_custom_stop_back_sure_click(lv_event_t *e)
{
    ...
    printf("[custom] stop_back sure -> major_menu\n");
}
...
void jump_to_custom_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->hour, menu->label_848);
        nav_blink_extra(menu->min, menu->label_849);
    ...
    lang_scr_load_anim(custom_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    g_send.cook_mode = MODE_FROZEN_BAKE;
    printf("[custom] jump: frozencook -> custom_menu\n");
}

void jump_to_custom_set(void)
{
    g_delay_source_page = PAGE_CUSTOM_SET;   /* 记录来源子类：收藏卡片名/启动分发依赖 */
    ...
    lang_scr_load_anim(custom_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] jump: menu -> custom_set\n");
}
...
void jump_to_custom_cooking(void)
{
    ...
    lang_scr_load_anim(custom_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[custom] jump: set -> custom_cooking\n");
}
...
void jump_to_custom_setting(void)
{
    ...
    if (set) {
        ...
        edit_register(set->min, set->minline, NULL,
                      &set_min, 0, 59, 1, "%02d");
        /* blink extras: 单位/方向箭头(离线推导,最近值归属) */
        nav_blink_extra(set->hour, set->label_875);
        nav_blink_extra(set->min, set->label_877);
...
void jump_to_custom_stop(void)
{
    ...
    lang_scr_load_anim(custom_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[custom] jump: cooking -> stop (pause)\n");
}
...
void jump_to_custom_stop_back(void)
{
    ...
    if (back) {
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_882, tr("烹饪中..."));

        if (g_complete_to_stop_back) {
            g_complete_to_stop_back = 0;
            if (g_keepwarm_active)
                lv_label_set_text(back->label_882, tr("保温中..."));
            else
                lv_label_set_text(back->label_882, tr("已完成"));
            lv_bar_set_value(back->bar_70, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_882, tr("预约中..."));
            lv_label_set_text(back->label_884, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
    ...
    lang_scr_load_anim(custom_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] stop/cooking -> stop_back\n");
}
...
void custom_resume_cooking(void)
{
    ...
    printf("[custom] resume: stop -> cooking\n");
}
...
static void on_custom_setting_sure_click(lv_event_t *e)
{
    ...
    g_send.set_temp = set_temp;
    g_send.remaining_ms = cook_total_ms;
    printf("[custom] setting sure -> cooking\n");
}
...
void jump_to_custom_complete(void)
{
    ...
    printf("[custom] jump: cooking -> complete\n");
...
void custom_rebuild_menu(page_id_t child)
{
    ...
    lang_scr_load_anim(custom_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] back to custom_menu\n");
}
...
void custom_rebuild_set(page_id_t child)
{
    ...
    lang_scr_load_anim(custom_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] back to custom_set\n");
}
...
void custom_rebuild_cooking(page_id_t child)
{
    ...
    lang_scr_load_anim(custom_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] back to custom_cooking\n");
}
...
void custom_rebuild_setting(void)
{
    ...
    lang_scr_load_anim(custom_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] back to custom_setting\n");
}
...
void custom_rebuild_stop(void)
{
    ...
    lang_scr_load_anim(custom_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] back to custom_stop\n");
    g_send.iface_status = IFACE_PAUSE;
}
...
void custom_rebuild_stop_back(void)
{
    ...
    lang_scr_load_anim(custom_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] back to custom_stop_back\n");
}
...
void custom_rebuild_complete(void)
{
    ...
    lang_scr_load_anim(custom_complete_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[custom] back to custom_complete\n");
}
