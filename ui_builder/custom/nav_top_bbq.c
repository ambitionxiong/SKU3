...
int top_setting_saved_temp, top_setting_saved_hour, top_setting_saved_min;

static void on_top_bbq_menu_next_click(lv_event_t *e);
static void on_top_bbq_set_sure_click(lv_event_t *e);
static void on_top_bbq_cooking_stop_click(lv_event_t *e);
static void on_top_bbq_cooking_setting_click(lv_event_t *e);
static void on_top_bbq_setting_sure_click(lv_event_t *e);
static void on_top_bbq_stop_start_click(lv_event_t *e);
static void on_top_bbq_stop_back_sure_click(lv_event_t *e);
static void on_top_bbq_edit_focus(lv_event_t *e);
void update_top_bbq_dir_icon(top_bbq_setting_t *set);
static void top_bbq_set_status(lv_obj_t *label, int temp, int hour, int min);
static void top_bbq_preheat_toggle(lv_event_t *e);
static void top_bbq_delay_toggle(lv_event_t *e);
static void top_bbq_contain_toggle(lv_event_t *e);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 顶部烧烤 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 顶部烧烤 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
static void top_bbq_preheat_toggle(lv_event_t *e)
{
    ...
    if (preheat_on) {
        ...
        lv_obj_clear_flag(set->onpreheat_button, LV_OBJ_FLAG_HIDDEN);
        lv_group_focus_obj(set->onpreheat_button);
    } else {
        lv_obj_add_flag(set->onpreheat_button, LV_OBJ_FLAG_HIDDEN);
...
static void top_bbq_delay_toggle(lv_event_t *e)
{
    ...
    /* ondelay：直接关闭 */
    delay_on = 0;
    lv_obj_add_flag(set->ondelay_button, LV_OBJ_FLAG_HIDDEN);
...
static void top_bbq_contain_toggle(lv_event_t *e)
{
    ...
    if (contain_on) {
        ...
        lv_obj_clear_flag(set->oncontain_button, LV_OBJ_FLAG_HIDDEN);
        lv_group_focus_obj(set->oncontain_button);
    } else {
        lv_obj_add_flag(set->oncontain_button, LV_OBJ_FLAG_HIDDEN);
...
void jump_to_top_bbq_menu(void)
{
    ...
    if (menu) {
        lv_obj_t *btns[] = {
            menu->temp_label, menu->hour_label, menu->min_label,
            menu->next_button,
        };
        if (g_top_bbq_menu) lv_group_del(g_top_bbq_menu);
        ...
        edit_register(menu->hour_label, menu->hourline, NULL,
                      &set_hour, 0, 4, 1, "%02d");
        edit_register(menu->min_label, menu->minline, NULL,
                      &set_min, 0, 59, 1, "%02d");
        /* blink extras: 单位/方向箭头(离线推导,最近值归属) */
        nav_blink_extra(menu->temp_label, menu->label_44);
        nav_blink_extra(menu->hour_label, menu->label_47);
        nav_blink_extra(menu->min_label, menu->label_48);

        lv_obj_add_event_cb(menu->temp_label, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->hour_label, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min_label, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next_button, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        ...
        if (menu->next_button)
            lv_group_focus_obj(menu->next_button);
    }
...
void jump_to_top_bbq_set(void)
{
    ...
    if (set) {
        lv_obj_t *btns[] = {
            set->sure_button,
            set->offpreheat, set->onpreheat_button,
            set->offdelay, set->ondelay_button,
            set->offcontain, set->oncontain_button,
        };
        if (g_top_bbq_set) lv_group_del(g_top_bbq_set);
        g_top_bbq_set = group_create_for_page(btns, 7);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offpreheat, top_bbq_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->onpreheat_button, top_bbq_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offdelay, top_bbq_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay_button, top_bbq_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offcontain, top_bbq_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->oncontain_button, top_bbq_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->sure_button, on_top_bbq_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
    }
...
void jump_to_top_bbq_cooking(void)
{
    ...
    if (cook) {
        ...
        if (g_top_bbq_cooking) lv_group_del(g_top_bbq_cooking);
        g_top_bbq_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_top_bbq_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->little, on_top_bbq_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        top_bbq_set_status(cook->statu, set_temp, set_hour, set_min);
...
void jump_to_top_bbq_setting(void)
{
    ...
    page_push(PAGE_TOP_BBQ_SETTING);
    lv_obj_clean(lv_scr_act());
    top_bbq_setting_create(&ui_manager);

    top_bbq_setting_t *set = top_bbq_setting_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->temp, set->hour, set->min, set->surebutton };
        if (g_top_bbq_setting) lv_group_del(g_top_bbq_setting);
        ...
        nav_blink_extra(set->hour, set->label_93);
        nav_blink_extra(set->min, set->label_94);

        lv_obj_add_event_cb(set->temp, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->surebutton, on_top_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->surebutton, on_top_bbq_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        lv_obj_add_flag(set->icon3, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(set->line2, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(set->icon2, LV_OBJ_FLAG_HIDDEN);
...
void jump_to_top_bbq_stop(void)
{
    ...
    if (stop) {
        ...
        if (g_top_bbq_stop) lv_group_del(g_top_bbq_stop);
        g_top_bbq_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_top_bbq_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_top_bbq_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        top_bbq_set_status(stop->statu, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_7, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
...
void jump_to_top_bbq_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_TOP_BBQ_COOKING) {
        top_bbq_cooking_t *cook = top_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_6) cooking_bar_val = lv_bar_get_value(cook->bar_6);
    }
    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_104, tr("保温中..."));
            else
                lv_label_set_text(back->label_104, tr("已完成"));
            lv_bar_set_value(back->bar_8, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_104, tr("预约中..."));
            lv_label_set_text(back->label_106, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_8, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_49, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->little, LV_OBJ_FLAG_HIDDEN);
        }

        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_104, tr("烹饪中..."));
    }
...
void top_bbq_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_6, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_6, cook_bar_saved, LV_ANIM_OFF);
...
static void on_top_bbq_setting_sure_click(lv_event_t *e)
{
    ...
    if (depth > 0 && page_stack[depth - 1] == PAGE_TOP_BBQ_STOP)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_TOP_BBQ_COMPLETE)
        depth--;
    lv_obj_clean(lv_scr_act());
    top_bbq_cooking_create(&ui_manager);
...
void jump_to_top_bbq_complete(void)
{
    ...
    {
        top_bbq_complete_t *cook = top_bbq_complete_get(&ui_manager);
        if (cook) {
            lv_obj_t *btns[] = { cook->button_27 };
            if (g_top_bbq_complete) lv_group_del(g_top_bbq_complete);
            g_top_bbq_complete = group_create_for_page(btns, 1);
            lv_obj_add_event_cb(cook->button_27, on_top_bbq_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            if (set_hour == 0)
                lv_label_set_text_fmt(cook->label_107, tr("| 顶部烧烤 | %d℃ | %02d分钟"), set_temp, set_min);
            else
                lv_label_set_text_fmt(cook->label_107, tr("| 顶部烧烤 | %d℃ | %d小时%02d分钟"), set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_9, 100, LV_ANIM_OFF);
        }
    }
...
void top_bbq_rebuild_set(page_id_t child)
{
    ...
    if (set_hour == 0) {
        ...
        if (child == PAGE_TOP_BBQ_COOKING && set->sure_button)
            lv_group_focus_obj(set->sure_button);
    }
...
void top_bbq_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->temp, on_top_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_top_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_top_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->surebutton, on_top_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->surebutton, on_top_bbq_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
...
{
    lv_obj_add_event_cb(btn, on_top_bbq_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
