...
void jump_to_lasagna_menu(void)
{
    ...
    if (menu) {
        ...
        nav_blink_extra(menu->hour, menu->label_637);
        nav_blink_extra(menu->min, menu->label_638);
...
void jump_to_lasagna_setting(void)
{
    ...
    if (set) {
        ...
        edit_register(set->min, set->minline, NULL,
                      &set_min, 0, 59, 1, "%02d");
        /* blink extras: 单位/方向箭头(离线推导,最近值归属) */
        nav_blink_extra(set->hour, set->label_668);
        nav_blink_extra(set->min, set->label_669);
...
void jump_to_lasagna_stop_back(void)
{
    ...
    if (back) {
        ...
        if (g_delay_cancel_to_stop_back) {
            ...
            lv_label_set_text(back->label_679, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
