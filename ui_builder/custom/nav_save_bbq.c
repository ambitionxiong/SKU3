...
int save_bbq_setting_saved_temp, save_bbq_setting_saved_hour, save_bbq_setting_saved_min;


static void on_save_bbq_menu_next_click(lv_event_t *e);
static void on_save_bbq_set_sure_click(lv_event_t *e);
static void on_save_bbq_cooking_stop_click(lv_event_t *e);
static void on_save_bbq_cooking_setting_click(lv_event_t *e);
static void on_save_bbq_setting_sure_click(lv_event_t *e);
static void on_save_bbq_stop_start_click(lv_event_t *e);
void save_bbq_resume_cooking(void);
static void on_save_bbq_stop_back_sure_click(lv_event_t *e);
static void on_save_bbq_edit_focus(lv_event_t *e);
void update_save_bbq_dir_icon(save_bbq_setting_t *set);
static void save_bbq_set_status(lv_obj_t *label, int temp, int hour, int min);
static void save_bbq_preheat_toggle(lv_event_t *e);
static void save_bbq_delay_toggle(lv_event_t *e);
static void save_bbq_contain_toggle(lv_event_t *e);
...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, tr("| 节能热风 | %d℃ | %02d分钟"), temp, min);
    else
        lv_label_set_text_fmt(label, tr("| 节能热风 | %d℃ | %d小时%02d分钟"), temp, hour, min);
}
...
static void on_save_bbq_stop_back_sure_click(lv_event_t *e)
{
    ...
    printf("[save_bbq] stop_back sure -> major_menu\n");
}
...
void jump_to_save_bbq_menu(void)
{
    ...
    page_push(PAGE_SAVE_BBQ_MENU);
    lv_obj_clean(lv_scr_act());
    save_bbq_menu_create(&ui_manager);
    ...
    if (menu) {
        ...
        if (g_save_bbq_menu) lv_group_del(g_save_bbq_menu);
        g_save_bbq_menu = group_create_for_page(btns, 4);

        edit_clear();
        edit_register(menu->temp, menu->line2, menu->line3,
                      &set_temp, 125, 250, 5, "%d");
        edit_register(menu->hour, menu->hourline, NULL,
                      &set_hour, 0, 4, 1, "%02d");
        edit_register(menu->min, menu->minline, NULL,
                      &set_min, 0, 59, 1, "%02d");
        /* blink extras: 单位/方向箭头(离线推导,最近值归属) */
        nav_blink_extra(menu->temp, menu->label_245);
        nav_blink_extra(menu->hour, menu->label_248);
        nav_blink_extra(menu->min, menu->label_249);

        lv_obj_add_event_cb(menu->temp, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->hour, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->min, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(menu->next, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);

        if (menu->next)
            lv_obj_add_event_cb(menu->next, on_save_bbq_menu_next_click,
    ...
    printf("[save_bbq] jump: cookmenu -> save_bbq_menu\n");
}
...
void jump_to_save_bbq_set(void)
{
    ...
    if (set) {
        ...
        if (g_save_bbq_set) lv_group_del(g_save_bbq_set);
        g_save_bbq_set = group_create_for_page(btns, 7);
    ...
    if (set_hour == 0) {
        ...
        lv_obj_add_event_cb(set->offpreheat, save_bbq_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->onpreheat, save_bbq_preheat_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offdelay, save_bbq_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->ondelay, save_bbq_delay_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->offcontain, save_bbq_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->oncontain, save_bbq_contain_toggle, LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(set->sure, on_save_bbq_set_sure_click,
                            LV_EVENT_CLICKED, NULL);
    }
...
void jump_to_save_bbq_cooking(void)
{
    ...
    page_push(PAGE_SAVE_BBQ_COOKING);
    lv_obj_clean(lv_scr_act());
    save_bbq_cooking_create(&ui_manager);

    save_bbq_cooking_t *cook = save_bbq_cooking_get(&ui_manager);
    if (cook) {
        lv_obj_t *btns[] = { cook->stop, cook->little };
        if (g_save_bbq_cooking) lv_group_del(g_save_bbq_cooking);
        g_save_bbq_cooking = group_create_for_page(btns, 2);

        lv_obj_add_event_cb(cook->stop, on_save_bbq_cooking_stop_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(cook->little, on_save_bbq_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);

        save_bbq_set_status(cook->status, set_temp, set_hour, set_min);
        lv_label_set_text_fmt(cook->timelabel, "%02d:%02d:%02d", set_hour, set_min, 0);
    }

    cook_total_ms = (set_hour * 3600 + set_min * 60) * 1000;
    if (cook) {
        lv_bar_set_range(cook->bar_22, 0, 100);
        lv_bar_set_value(cook->bar_22, 3, LV_ANIM_OFF);

        lv_anim_t a;
        lv_anim_init(&a);
        lv_anim_set_var(&a, cook->bar_22);
    ...
    printf("[save_bbq] jump: set -> save_bbq_cooking\n");
}

// cooking → setting（不暂停 timer）
void jump_to_save_bbq_setting(void)
{
    save_bbq_setting_saved_temp = set_temp; save_bbq_setting_saved_hour = set_hour; save_bbq_setting_saved_min = set_min;
    page_push(PAGE_SAVE_BBQ_SETTING);
    lv_obj_clean(lv_scr_act());
    save_bbq_setting_create(&ui_manager);

    save_bbq_setting_t *set = save_bbq_setting_get(&ui_manager);
    if (set) {
        lv_obj_t *btns[] = { set->temp, set->hour, set->min, set->sure };
        if (g_save_bbq_setting) lv_group_del(g_save_bbq_setting);
        g_save_bbq_setting = group_create_for_page(btns, 4);
        ...
        nav_blink_extra(set->hour, set->label_18);
        nav_blink_extra(set->min, set->label_19);

        lv_obj_add_event_cb(set->temp, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_save_bbq_edit_focus,
                            LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_save_bbq_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
    ...
    printf("[save_bbq] jump: cooking -> save_bbq_setting\n");
}
...
void jump_to_save_bbq_stop(void)
{
    ...
    page_push(PAGE_SAVE_BBQ_STOP);
    lv_obj_clean(lv_scr_act());
    save_bbq_stop_create(&ui_manager);

    save_bbq_stop_t *stop = save_bbq_stop_get(&ui_manager);
    if (stop) {
        lv_obj_t *btns[] = { stop->start, stop->little };
        if (g_save_bbq_stop) lv_group_del(g_save_bbq_stop);
        g_save_bbq_stop = group_create_for_page(btns, 2);
        lv_obj_add_event_cb(stop->start, on_save_bbq_stop_start_click,
                            LV_EVENT_CLICKED, NULL);
        lv_obj_add_event_cb(stop->little, on_save_bbq_cooking_setting_click,
                            LV_EVENT_CLICKED, NULL);
        ...
        save_bbq_set_status(stop->status, set_temp, set_hour, set_min);

        lv_bar_set_range(stop->bar_23, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
    ...
    printf("[save_bbq] jump: cooking -> stop (pause)\n");
}
...
void jump_to_save_bbq_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_SAVE_BBQ_COOKING) {
        save_bbq_cooking_t *cook = save_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_22) cooking_bar_val = lv_bar_get_value(cook->bar_22);
    }

    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_save_bbq_complete;
    page_push(PAGE_SAVE_BBQ_STOP_BACK);
    lv_obj_clean(lv_scr_act());
    save_bbq_stop_back_create(&ui_manager);

    save_bbq_stop_back_t *back = save_bbq_stop_back_get(&ui_manager);
    if (back) {
        lv_obj_t *btns[] = { back->sure };
        if (g_save_bbq_stop_back) lv_group_del(g_save_bbq_stop_back);
        g_save_bbq_stop_back = group_create_for_page(btns, 1);
        lv_obj_add_event_cb(back->sure, on_save_bbq_stop_back_sure_click,
                            LV_EVENT_CLICKED, NULL);

        save_bbq_set_status(back->status, set_temp, set_hour, set_min);
        ...
        if (p > 100) p = 100;
        lv_bar_set_range(back->bar_24, 0, 100);
                lv_bar_set_value(back->bar_24, p, LV_ANIM_OFF);
        if (g_complete_to_stop_back) {
            g_complete_to_stop_back = 0;
            if (g_keepwarm_active)
                lv_label_set_text(back->label_276, tr("保温中..."));
            else
                lv_label_set_text(back->label_276, tr("已完成"));
            lv_bar_set_value(back->bar_24, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_276, tr("预约中..."));
            lv_label_set_text(back->label_278, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_24, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_126, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->button_87, LV_OBJ_FLAG_HIDDEN);
        }

        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_276, tr("烹饪中..."));
    }
...
void save_bbq_resume_cooking(void)
{
    ...
    if (cook) {
        ...
        lv_bar_set_range(cook->bar_22, 0, 100);
        if (cook_bar_saved > 100) cook_bar_saved = 100;
        lv_bar_set_value(cook->bar_22, cook_bar_saved, LV_ANIM_OFF);
    ...
    printf("[save_bbq] resume: stop -> cooking\n");
}
...
static void on_save_bbq_setting_sure_click(lv_event_t *e)
{
    ...
    if (depth > 1) depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_SAVE_BBQ_STOP)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_SAVE_BBQ_COMPLETE)
        depth--;
    lv_obj_clean(lv_scr_act());
    save_bbq_cooking_create(&ui_manager);
    ...
    g_send.set_temp = set_temp;
    g_send.remaining_ms = cook_total_ms;
    printf("[save_bbq] setting sure -> cooking\n");
}
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_SAVE_BBQ_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_SAVE_BBQ_STOP)
        depth--;
    page_push(PAGE_SAVE_BBQ_COMPLETE);
    lv_obj_clean(lv_scr_act());
    save_bbq_complete_create(&ui_manager);

    {
        save_bbq_complete_t *cook = save_bbq_complete_get(&ui_manager);
        if (cook) {
            lv_obj_t *btns[] = { cook->button_89 };
            if (g_save_bbq_complete) lv_group_del(g_save_bbq_complete);
            g_save_bbq_complete = group_create_for_page(btns, 1);
            lv_obj_add_event_cb(cook->button_89, on_save_bbq_cooking_setting_click,
                                LV_EVENT_CLICKED, NULL);
            save_bbq_set_status(cook->status, set_temp, set_hour, set_min);
            lv_bar_set_value(cook->bar_25, 100, LV_ANIM_OFF);
        }
    }
    ...
    g_send.remaining_ms = 0;
        g_send.iface_status = IFACE_COMPLETE;
    g_send.remaining_ms = 0;
    printf("[save_bbq] jump: cooking -> complete\n");
...
void save_bbq_rebuild_set(page_id_t child)
{
    ...
    if (set_hour == 0) {
        ...
        if (child == PAGE_SAVE_BBQ_COOKING && set->sure)
            lv_group_focus_obj(set->sure);
    }
...
void save_bbq_rebuild_cooking(page_id_t child)
{
    ...
    if (cook) {
        ...
        if (child == PAGE_SAVE_BBQ_SETTING) {
            ...
            lv_bar_set_value(cook->bar_22, progress, LV_ANIM_OFF);
            lv_anim_t a;
            lv_anim_init(&a);
            lv_anim_set_var(&a, cook->bar_22);
            lv_anim_set_exec_cb(&a, anim_bar_set_value);
...
void save_bbq_rebuild_setting(void)
{
    ...
    if (set) {
        ...
        lv_obj_add_event_cb(set->temp, on_save_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->hour, on_save_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->min, on_save_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_save_bbq_edit_focus, LV_EVENT_FOCUSED, NULL);
        lv_obj_add_event_cb(set->sure, on_save_bbq_setting_sure_click,
                            LV_EVENT_CLICKED, NULL);
...
void save_bbq_rebuild_stop_back(void)
{
    ...
    if (back) {
        ...
        if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_SAVE_BBQ_COOKING) {
        save_bbq_cooking_t *cook = save_bbq_cooking_get(&ui_manager);
        if (cook && cook->bar_22) p = lv_bar_get_value(cook->bar_22);
    }
        if (p <= 0) {
            uint32_t elapsed = cook_timer ? (lv_tick_get() - cook_start_time) : cook_elapsed_saved;
            p = stop_back_progress(elapsed, cook_total_ms);
...
{
    lv_obj_add_event_cb(btn, on_save_bbq_cooking_setting_click, LV_EVENT_CLICKED, NULL);
}
