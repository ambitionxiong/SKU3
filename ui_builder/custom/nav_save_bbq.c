...
{
    if (hour == 0)
        lv_label_set_text_fmt(label, "| 节能热风 | %d℃ | %02d分钟", temp, min);
    else
        lv_label_set_text_fmt(label, "| 节能热风 | %d℃ | %d小时%02d分钟", temp, hour, min);
}
    ...
    if (back) {
        ...
        if (g_complete_to_stop_back) {
            ...
            if (g_keepwarm_active)
                lv_label_set_text(back->label_276, "保温中...");
            else
                lv_label_set_text(back->label_276, "已完成");
            lv_bar_set_value(back->bar_24, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_276, "预约中...");
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_276, "烹饪中...");
    }
