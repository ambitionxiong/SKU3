...
static void on_pizza3_stop_back_sure_click(lv_event_t *e)
{
    ...
    printf("[pizza3] stop_back sure -> major_menu\n");
}
...
void jump_to_pizza3_menu(void)
{
    ...
    lang_scr_load_anim(pizza3_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    g_send.cook_mode = MODE_FROZEN_BAKE;
    printf("[pizza3] jump: frozencook -> pizza3_menu\n");
}
...
void jump_to_pizza3_set(void)
{
    ...
    if (set_hour == 0) {
        ...
        lv_obj_set_pos(set->label_778, 365 + lang_dyn_dx(), 269 + lang_dyn_dy());
    } else {
        ...
        lv_obj_set_pos(set->label_778, 448 + lang_dyn_dx(), 269 + lang_dyn_dy());
    }
    ...
    lang_scr_load_anim(pizza3_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] jump: menu -> pizza3_set\n");
}
...
void jump_to_pizza3_cooking(void)
{
    ...
    lang_scr_load_anim(pizza3_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[pizza3] jump: set -> pizza3_cooking\n");
}
...
void jump_to_pizza3_stop(void)
{
    ...
    lang_scr_load_anim(pizza3_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    ...
    printf("[pizza3] jump: cooking -> stop (pause)\n");
}
...
void jump_to_pizza3_stop_back(void)
{
    ...
    if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_PIZZA3_COOKING) {
        pizza3_cooking_t *cook = pizza3_cooking_get(&ui_manager);
        if (cook && cook->bar_60) cooking_bar_val = lv_bar_get_value(cook->bar_60);
    }
    g_on_stop_back = 1;
    ...
    if (back) {
        ...
        if (g_send.iface_status == IFACE_COOKING)
            lv_label_set_text(back->label_800, tr("烹饪中..."));

        if (g_complete_to_stop_back) {
            g_complete_to_stop_back = 0;
            if (g_keepwarm_active)
                lv_label_set_text(back->label_800, tr("保温中..."));
            else
                lv_label_set_text(back->label_800, tr("已完成"));
            lv_bar_set_value(back->bar_62, 100, LV_ANIM_OFF);
        }
        if (g_delay_cancel_to_stop_back) {
            g_delay_cancel_to_stop_back = 0;
            lv_label_set_text(back->label_800, tr("预约中..."));
            lv_label_set_text(back->label_802, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
            lv_obj_add_flag(back->bar_62, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(back->image_368, LV_OBJ_FLAG_HIDDEN);
    ...
    lang_scr_load_anim(pizza3_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] stop/cooking -> stop_back\n");
}
...
void pizza3_resume_cooking(void)
{
    ...
    printf("[pizza3] resume: stop -> cooking\n");
}
...
static void on_pizza3_setting_sure_click(lv_event_t *e)
{
    ...
    g_send.set_temp = set_temp;
    g_send.remaining_ms = cook_total_ms;
    printf("[pizza3] setting sure -> cooking\n");
}
...
void jump_to_pizza3_complete(void)
{
    ...
    printf("[pizza3] jump: cooking -> complete\n");
...
void pizza3_rebuild_menu(page_id_t child)
{
    ...
    lang_scr_load_anim(pizza3_menu_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] back to pizza3_menu\n");
}
...
void pizza3_rebuild_set(page_id_t child)
{
    ...
    lang_scr_load_anim(pizza3_set_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] back to pizza3_set\n");
}
...
void pizza3_rebuild_cooking(page_id_t child)
{
    ...
    lang_scr_load_anim(pizza3_cooking_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] back to pizza3_cooking\n");
}
...
void pizza3_rebuild_setting(void)
{
    ...
    lang_scr_load_anim(pizza3_setting_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] back to pizza3_setting\n");
}
...
void pizza3_rebuild_stop(void)
{
    ...
    lang_scr_load_anim(pizza3_stop_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] back to pizza3_stop\n");
    g_send.iface_status = IFACE_PAUSE;
}
...
void pizza3_rebuild_stop_back(void)
{
    ...
    lang_scr_load_anim(pizza3_stop_back_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] back to pizza3_stop_back\n");
}
...
void pizza3_rebuild_complete(void)
{
    ...
    lang_scr_load_anim(pizza3_complete_get(&ui_manager)->obj,
                     LV_SCR_LOAD_ANIM_NONE, 0, 0,
                     ui_manager.auto_del);
    printf("[pizza3] back to pizza3_complete\n");
}
