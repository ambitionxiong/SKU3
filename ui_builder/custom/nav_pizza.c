...
static void on_pizza_stop_back_sure_click(lv_event_t *e)
{
    ...
    set_temp = 210; set_temp_up = 210; set_temp_down = 210; set_hour = 0; set_min = 30;
...
{
    set_temp = 210; set_hour = 0; set_min = 30;
    ...
    if (menu) {
        ...
        edit_register_temp(menu->temp, menu->templine2, menu->templine3,
                      &set_temp, 200, 230, 5, "%d");
        edit_register(menu->hour, menu->hourline, NULL,
                      &set_hour, 0, 4, 1, "%02d");
        edit_register(menu->min, menu->minline, NULL,
                      &set_min, 0, 59, 1, "%02d");
        /* blink extras: 单位/方向箭头(离线推导,最近值归属) */
        nav_blink_extra(menu->temp, menu->label_195);
        nav_blink_extra(menu->hour, menu->label_198);
        nav_blink_extra(menu->min, menu->label_199);
...
{
    g_delay_source_page = PAGE_PIZZA_SET;   /* 记录来源子类：收藏卡片名/启动分发依赖 */
...
void jump_to_pizza_setting(void)
{
    ...
    if (set) {
        ...
        nav_blink_extra(set->hour, set->label_229);
        nav_blink_extra(set->min, set->label_230);
...
{
    edit_clear();
    int cooking_bar_val = 0; if (cook_timer && depth > 0 && page_stack[depth - 1] == PAGE_PIZZA_COOKING) {
        pizza_cooking_t *cook = pizza_cooking_get(&ui_manager);
        if (cook && cook->bar_12) cooking_bar_val = lv_bar_get_value(cook->bar_12);
    }
    g_on_stop_back = 1;
    g_stop_back_complete = jump_to_pizza_complete;
    ...
    if (back) {
        ...
        g_pizza_stop_back = group_create_for_page(btns, 1);
        ...
        if (g_delay_cancel_to_stop_back) {
            ...
            lv_label_set_text(back->label_238, g_delay_cancel_btn ? tr("回到上一页") : tr("回到主页"));
...
{
    edit_clear();
    if (depth > 0 && page_stack[depth - 1] == PAGE_PIZZA_STOP_BACK)
        depth--;
    if (depth > 0 && page_stack[depth - 1] == PAGE_PIZZA_STOP)
        depth--;
